...
class TestEvaporation:
    def test_one_half_life_halves_heat(self, vine_rw, ranger):
        r, clock = ranger
        vine_rw.trails.add_heat(["concepts/rag"], amount=0.8)
        vine_rw.trails.set_updated("concepts/rag", clock())
        clock.advance(30 * DAY)  # exactly one default half-life
        r.evaporate()
        assert abs(vine_rw.trails.get_heat("concepts/rag") - 0.4) < 0.01

    def test_dust_rows_vanish(self, vine_rw, ranger):
        r, clock = ranger
        vine_rw.trails.add_heat(["concepts/rag"], amount=0.05)
        vine_rw.trails.set_updated("concepts/rag", clock())
        clock.advance(120 * DAY)  # 4 half-lives: 0.05 -> ~0.003 < 0.01 floor
        report = r.evaporate()
        assert report["removed"] == 1
        assert vine_rw.trails.get_heat("concepts/rag") == 0.0
    ...
    def test_back_to_back_runs_are_idempotent(self, vine_rw, ranger):
        ...
        r.evaporate()
        h1 = vine_rw.trails.get_heat("concepts/rag")
        r.evaporate()  # same clock instant: must not decay again
        assert vine_rw.trails.get_heat("concepts/rag") == h1

    def test_stale_sessions_cleared(self, vine_rw, ranger):
        r, clock = ranger
        vine_rw.trails.add_heat(["concepts/rag"], amount=0.5, scope="hunt-42")
        vine_rw.trails.set_updated("concepts/rag", clock(), scope="hunt-42")
        clock.advance(25 * 3600)  # past the 24h TTL
        report = r.evaporate()
        assert report["stale_sessions_cleared"] == 1
        assert vine_rw.trails.get_heat("concepts/rag", session="hunt-42") == 0.0


class TestLinkTending:
    SHORTCUT = {"rel": "discovered-shortcut", "target": "concepts/rag",
                "confidence": 0.5, "discovered_by": "agent"}

    def _add_shortcut(self, vine, source="projects/mixerllm/architecture"):
    ...
    def test_hot_proposal_is_promoted_with_audited_commit(self, vine_rw, ranger, forest_rw):
        ...
        vine_rw.trails.add_heat([src, "concepts/rag"], amount=0.4)  # well used
        report = r.tend_links()
        assert report["promoted"] == [f"{src} discovered-shortcut->concepts/rag"]
    ...
    def test_cold_proposal_is_pruned(self, vine_rw, ranger, clock):
        ...
        r.evaporate()
        report = r.tend_links()
        assert report["pruned"] == [f"{src} discovered-shortcut->concepts/rag"]
    ...
    def test_warm_proposal_is_left_alone(self, vine_rw, ranger):
        ...
        r, _ = ranger
        src = self._add_shortcut(vine_rw)
        vine_rw.trails.add_heat([src, "concepts/rag"], amount=0.1)  # below floor
        report = r.tend_links()
    ...
    def test_full_confidence_links_are_never_touched(self, vine_rw, ranger, clock):
        ...
        r, _ = ranger
        node = vine_rw.forest.read("projects/mixerllm/architecture")
        ...
        report = r.tend_links()
        assert report["pruned"] == []
        assert (vine_rw.forest.read("projects/mixerllm/architecture")
                .frontmatter.get("links") or []) == before

    def test_promotion_is_idempotent(self, vine_rw, ranger):
        r, _ = ranger
        src = self._add_shortcut(vine_rw)
        vine_rw.trails.add_heat([src, "concepts/rag"], amount=0.4)
...
class TestHealth:
    def test_health_flags_problems(self, vine_rw, ranger, forest_rw):
        ...
        fat = forest_rw / "sales" / "_index.md"
        body = fat.read_text(encoding="utf-8")
        extra = "\n".join(f"- [[sales/fake-{i}]] — fake entry." for i in range(160))
        ...
        vine_rw.graft("concepts/rag", {"set_frontmatter": {"tags": ["x"]}})  # touch nothing relevant
        node_path = forest_rw / "concepts" / "rag.md"
        text = node_path.read_text(encoding="utf-8")
        node_path.write_text(text.replace("tags:", "source_path: gone.md\ntags:", 1),
                             encoding="utf-8")

        report = r.health()
        assert "sales/_index" in report["needs_split"]
        assert "concepts/rag" in report["stale_passports"]
        assert report["lint"]["errors"] >= 0 and "heat" in report

    def test_uncertain_inventory_buckets(self, vine_rw, ranger):
        r, _ = ranger
        vine_rw.graft("projects/mixerllm/architecture", {"add_links": [
