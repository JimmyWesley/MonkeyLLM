# MonkeyLLM Technical Specification v0.14 (Phase 0/1/2 + host layer)
...
A web console served by the Station, offering: forest browser (tree,
passport, body, links, per-node git history); search console (locate/sniff
with the budgets the agent sees); dataset console (query manual, read-only
SQL runner, `tend` forms); ingestion console (adopt/sync runs, converter
status, stale report, and the review queue for G.4.2.1 edge proposals —
approve promotes, reject prunes); trails dashboard (heat, shortcuts,
promote/prune history, session replay); governance console (principals,
roles, policies, tokens, audit); health (Ranger reports, snapshots).

Studio MUST consume only the documented REST surface. It MUST NOT hold a
privileged side-channel: whatever Studio can do, an API client with the
same principal can do and whatever a principal cannot do, Studio cannot
show.
