# MonkeyLLM — Technical Specification v0.18 (Phase 0/1/2 + host layer)
...
storing it would give a rotation two places to go wrong. At startup it is
granted `admin` on every forest in the registry. If the variables are
absent, the door simply does not exist — a deployment that never sets them
has no default password, which is the only safe default.
...
| | Tokens | which credentials exist, and until when |
| | Audit | who saw what |
...
#### J.5.4 Tokens, and the panel that does not exist

The Tokens console issues, lists and revokes API keys under J.2.2: label,
principal, expiry, last use, prefix. The secret appears once, at creation,
and the console MUST say so at the moment it is shown rather than
afterwards.
