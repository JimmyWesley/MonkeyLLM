# MonkeyLLM — Technical Specification v0.44 (Phase 0/1/2 + host layer)
...
3. **Bounded by construction, and never silently.** ≤ 3 rows per table;
   each cell clipped to 120 characters with `…`; BLOBs rendered as
   `<blob N bytes>`, never as their bytes; NULL as an empty cell; pipes
   and newlines escaped so the table stays a table. At most **20 tables**
   get a sample section, and when more exist the section MUST end with a
   line stating how many were omitted. A map that quietly stops is worse
   than a short one.
...
sheet is its header; empty sheets are skipped.

A workbook whose sheet count exceeds the C.7.1 table limit is refused with
`E_SCHEMA` naming the file and the limit — it is a conversion the Gardener
reports per-file, not a crash and not a silent first-sheet-only import.
Taking sheet one and dropping the rest is how a spreadsheet arrives in a
forest missing the data somebody adopted it for.
...
**SQL is coloured where it is typed.** The console already colours every
literal surface it *prints* (statements pending a `tend`, stored DDL,
fenced blocks in an answer). The one surface it did not colour is the
editable one, which is the surface a person actually reads while composing
a statement. A plain `<textarea>` cannot hold spans, so the editor is a
