# MonkeyLLM Technical Specification v0.58 (Phase 0/1/2 + host layer)
...
- `transplant` joins the J.16 events (`node.transplanted`, identity
  only), the signature table, both MCP surfaces (19 tools; the J.1.2
...
4. **A read of the old id answers `E_MOVED`, naming `moved_to`** — a
   ...
   truth when the other half is known. HTTP 404 (it is not at this
