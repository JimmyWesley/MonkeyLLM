# MonkeyLLM — Technical Specification v0.46 (Phase 0/1/2 + host layer)
...
rows. What it cannot supply is **meaning**: that `valor_total_invoice` is
in USD while `valor_cambio` is in BRL, that `status` uses one-letter
codes, that direct imports are the rows where `arrendatario` is null, or
which join is the one that answers the question people actually ask.
...
6. **`harvest` MUST carry it too (amended C.6c).** There are two paths to
   an answer and `look` is on only one of them: the walk calls primitives,
   but the sweep is `locate` + `sniff` + the matched sections and never
   looks at anything. A teaching visible to the walk alone is invisible
   from the console's ordinary ask, which is where it will actually be
   written. A dataset result therefore carries `notes` in its item —
   independent of whether the section happened to match the question's
   terms, because whether a person's instructions about how to read the
   data share vocabulary with today's question is not a reason to withhold
   them.
