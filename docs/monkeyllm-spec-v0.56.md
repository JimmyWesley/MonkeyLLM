# MonkeyLLM Technical Specification v0.56 (Phase 0/1/2 + host layer)
...
- **The passport says who and when (v0.56):** `created` and `source`
  are returned always, `aliases` whenever the node carries any. All three
...
4. **`section` accepts a list** (≤ 10 names, one call, one 4,000-token
   ...
   `sections: [{section, body, body_tokens}]` in request order, plus
