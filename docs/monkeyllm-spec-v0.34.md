# MonkeyLLM — Technical Specification v0.34 (Phase 0/1/2 + host layer)
...
terms, the effective `k`, hops, binding, or scope is a miss. A run with
empty evidence, asked twice, is two provider calls and no entry.
`cache: false` answers
