# MonkeyLLM Technical Specification v0.83 (Phase 0/1/2 + host layer)
...
**Changelog v0.82 → v0.83 — the extension that could not be used.**

An operator uploaded two extensions through the console Part L had given
them, saw both listed, enabled both on the forest — and then could not use
either. The PDF converter was installed and the ingest console still greyed
out every `.pdf` in the file picker; the transcriber registered a role and
the Models console had no card for it, because the console's list of roles
was a constant and the bind route refused any name outside it. Nothing had
failed: the mechanism shipped in v0.80 and the surfaces it changed had never
been told. The same operator reported the install box itself: a file chosen
at the bottom of the card and a *Review* button at its top right, neither
one saying it was the next step, and a review that opened a dialog to
install from — three moves for one act, none of them adjacent.

- **The chain can be asked what it takes (amends G.2).** The engine
  answers, in discovery order, which file extensions the forest's
  converter chain claims and who claims each — a command hook, an
  extension, the injected describer, a plugin, a built-in. One function,
  read by every surface that needs the list, so the list can never be
  copied.
- **The console accepts what the forest converts (new J.8.5).** The GET
  beside the ingest POST carries `formats`; the console's picker, its drop
  filter and its hint are derived from it, and a file refused by type
  names the formats the forest takes. An extension enabled on a forest is
  usable on it the moment it is loaded, with nobody editing a list.
- **A registered role is bindable wherever a role is bound (L.6 rule 1,
  amends J.10).** `GET /v1/admin/models` carries the bindable roles with
  their `kind`, their origin and a description; `POST` accepts every role a
  loaded extension registers; the console renders the list from the host
  and shapes the form by `kind`, because a transcription binding has no
  reply length and no reasoning switch. A role MAY carry a `description`
  in the manifest (amends L.1), which is what the card says about it.
- **A first install is active at once (amends L.8).** The restart L.8
  requires is about unloading: Python cannot drop a module. An `id` this
  process has never loaded has nothing to unload, so installing it MAY
  activate it live, and the host says which of the two happened —
  `activated`, `restart_required` — and the listing says `loaded` per
  extension. Reinstalling or updating a loaded id still needs the restart.
- **The install is one legible act (amends L.11).** The primary action
  sits beside the thing it acts on and is the only primary control on the
  card; choosing a file is not an act, reviewing is, and the review is
  shown in place; installing from a forest's console MAY enable on that
  forest in the same act, by a visible pre-checked control, when the
  principal administers it; the outcome states what is active now, what
  needs a restart, and what is still missing to use it.
- **Acceptance F.213-F.218.**

**Changelog v0.81 → v0.82 — the ask that nobody could see.**

A consumer team wired an agent to a Station over MCP and reported that the
product had no way to ask a question. Their client's tool list did not carry
`answer`, so they built a REST client of their own to reach the walk — and
then asked for four things, every one of which was a gap on this side, and
none of which was the one they named first.

- **The question is a capability (new J.2.7; amends J.2.6, J.3, J.10.3,
  J.5.1, J.5.12).** `answer` rode `read` plus a model binding, so whoever
  could read could make the forest's model run, and nothing in a grant, a
  key or a menu said so. `answer` is now the seventh capability token:
  granted like the others, masked like the others, reported by `forests()`
  and `/v1/me`, and the MCP `answer` tool is listed only for a key that
  holds it somewhere it reaches — L.7's rule for a tool that could only
  refuse. Every grant and every stored mask that carried `read` gains
  `answer` once, by a data repair, so nobody's behaviour changes without
  somebody's decision; the pair ceiling grows to `{read, ingest, answer}`.
- **The walk is on both surfaces (amends J.10.5, J.10.12).** The MCP
  `answer` never carried `hops`. No rule said so: J.10.11's note that the
  walk is lane-bound and opt-in was read as a reason to keep it off a menu
  it was never about, and a consumer built a transport to reach it. `hops`
  rides the tool now, and a walk reports one protocol progress notification
  per hop when the client asked for progress — a walk runs past a minute,
  and a client's own clock is what ends it otherwise.
- **The answer at the size the caller can carry (new J.10.13).** `answer`
  handed back the bundle with every excerpt, the walk's `read` with every
  body it opened, and the trace — to a client that chose `answer` so that
  the forest's model would read and its own would not have to. `detail:
  "full" | "sources" | "answer"` shapes the response at the close; `full`
  is byte-identical to today, and the store, the audit and the events keep
  the whole record.
- **The reasoning is not the reply (amends J.10.8).** A model that thinks
  inline handed its `<think>` block to the reader as the answer, and the
  block spent the budget the prompt had stated. It is stripped at the one
  place the content is read, the reply says `reasoning_stripped`, and a
  walk's turn parses again.
- **The SDK's refusal wears the envelope (J.1.2 rule 8).** An argument the
  input schema refused came back as the SDK's own prose under `isError`,
  which is what forced the consumer to parse text. It is re-stated as the
  C.12 envelope, in the words of the same signature table REST reads.
- **The reference is a call (amends J.10.9, J.1.2 rule 7).** Over MCP a
...
way in. `docs/extending.md` still described the pre-Part-L world and cited
**v0.20**, so the only document naming extensions contradicted what had been
released; nothing anywhere taught how to write one; and the console's
install box asked for "a path", which is a path on the **host**. On a
Station reached through a browser that is the container's filesystem, so an
operator holding an extension they had just written had no route at all
short of publishing it to git first. The local-file door served only
somebody with a shell on the host — who would have used the CLI.

- **An uploaded archive is a source (amends L.2).** A fifth door into the
  same resolver: bytes arrive as `{name, b64}`, the shape ingest has used
  since J.8, and land in the workdir as the local archive L.2 already knows
  how to resolve. It is `unverified` **by construction** — an upload carries
  no forge and no index identity, so there is nothing to verify and the
  tier says so rather than implying a check happened. Bounded by
  `MONKEYLLM_STATION_EXT_UPLOAD_MAX_MB` (25 by default), because this door
  is for an extension's own code: dependencies are resolved at install, and
  a vendored-wheel bundle is the local-file route on the host.
- **A seam's handler has a declared contract (amends L.3, L.9 rule 1).**
  What a `curation` handler receives, what an `events` handler receives and
  what each must return were prose in this document and nowhere in the
  code, so the conformance kit could answer "it works" about an extension
  whose handler took the wrong parameters — the author found out on the
  first ingest, at whatever hour that ran. Each seam now DECLARES its
  contract, the kit CHECKS the handler's signature against it at install,
  and the authoring reference is generated from the same declaration.
- **The authoring surface (new L.16).** Prose that teaches is written by
  people and lives in the repository; the schemas are **derived** — the
  manifest's own JSON schema, the seam contracts, the role kinds, the
  config field types — and a hand-copied schema is a schema that lies the
  first time a seam is added. The console serves the derived half and hands
  out a folder for an author's agent, and the person who may WRITE an
  extension is not the person who may install one: authoring is offered to
  anyone signed in, exactly as J.5.12 reasoned about skills, while
  installation keeps L.7 rule 1's authority.
- **Acceptance F.199-F.205.**

**Changelog v0.79 → v0.80 — a capability the deployment does not have.**

Part G has said since v0.9 that "UIs and bots are MCP/library clients, not
plugins", and that sentence drew the line correctly while leaving a whole
class of work with nowhere to stand: not a client, not a contract change,
but a capability this deployment simply lacks — transcribing audio,
converting a format nobody here has heard of, reading a proprietary export.
Each arrives with a dependency, and the dependency is the problem. FFmpeg is
GPL or LGPL depending on the build; a converter drags a machine-learning
stack; a transcriber drags another. `LICENSING.md` splits this tree
Apache-2.0 / AGPL-3.0-only and that split holds only while the dependency
list stays clean, so pinning any of them makes this project their
distributor.

- **Part L (new).** An extension is a package that **contributes at named
  seams** (L.3) and never patches — which is what leaves this project free
  to refactor, since an extension is coupled to a published contract and not
  to the code behind it. `register(api)` is **enumerated** like `ScopedVine`
  (L.4): a new engine capability needs its own line before an extension can
  reach it. A handler marked `heavy` runs in the extension's own
  environment, in its own process (L.5), which is what keeps the motivating
  dependency out of the process that serves the forest.
- **This project distributes nothing (L.0, L.2).** The curated index carries
  signed *pointers*; what lands on an operator's machine is the operator's
  act, under a licence shown before the install completes. Four sources —
  index, git, archive, local file — through one resolver, and **whatever the
  source, it resolves to an immutable artifact before it is validated**: a
  git ref becomes a commit SHA, because a tag can be force-pushed and "the
  thing that was signed" must be the thing that runs. A moving ref is
  allowed and **marked**; it is `unverified` by construction, since a branch
  carries no tag to verify.
- **Three tiers, and what they cannot promise (L.2).** `verified` from the
  index, `signed` from a signed git tag checked against the forge's own
  published keys, `unverified` for everything else. Identity is delegated to
  the forge, so a compromised account is a compromised extension — stated,
  and answered by recording the identity that verified an install so a
  transfer or rotation surfaces on update. And resolving `requirements.txt`
  executes third-party build code by construction, so the conformance kit
  does **not** run before third-party code runs; it runs before anything is
  **registered**, and the documentation must say so in those words.
- **The mechanism is the engine's, the governance is the Station's (L.12).**
  The cut that already separates `Vine` from `ScopedVine`. It follows that
  an operator with no Station installs extensions — required, not
  incidental: a capability reachable only through the AGPL host would make
  every extension author a client of that host. Global install is a
  host-level directory; per-forest enablement is a line in the forest's
  `_meta/`, so it is versioned, travels in a snapshot, and `validate`
  reports an extension a forest expects and does not have. `_meta` declares
  expectation and never grants it.
- **A contribution the host cannot serve is inert and named, never an error
  (L.10).** J.1.2 rule 4 applied to extensions. The load-bearing half is
  configuration: **the config schema is the source of truth and a panel is
  decoration over it**, enforced by the kit, which is what keeps a
  console-less host first-class instead of degraded. The console surface is
  declarative (L.11) because J.5.13 admits no third-party script in the
  console's origin, and that rule is not relaxed for an extension.
- **Amends J.4.2, J.10, J.5.** The audit row gains **`via`** (`ext:<id>`),
  nullable, while `principal` stays whoever caused the act — a forensic
  question is *who asked for this*, and an extension is not an answer to it.
  A role gains a `kind` (`chat`/`embed`/`vision`/`transcribe`/`rerank`)
  because transcription is not `/chat/completions`, and an extension
  **registers** a role rather than carrying a key: `api.models` hands it a
  bound caller and never an endpoint or a credential, so extension spend is
  configured and metered where all other spend is. Quota is per extension
  per forest and exhaustion is a refusal, never silence.
- **Every seam is read by the host that declares it.** `converters` by the
  Gardener (above `extra_converters`, below an operator's own command hook —
  their `_meta/gardener.yaml` is the most local statement of intent there
  is), `curation` after the host's own hooks, `events` on the ONE emitter
  that already decides webhooks, `tools` on the MCP menu, `routes` under
  `/v1/ext/<id>/`, `jobs` pulled rather than scheduled (a scheduler here
  would be a second Ranger with a second failure mode), `ranking` inside
  `harvest` before the budget, `prompt` appended to both modes' system
  message, `roles` in the models console beside every other binding.
- **Acceptance F.174-F.198.**
...
2. A local `payload` MUST resolve inside the forest root and MUST exist
   ...
   `payload escapes the forest` and `payload not found: <payload>` are
...
**The chain can be asked what it takes (v0.83).** The same discovery that
ranks claimants for a file MUST be able to answer, without a Vine and
without opening anything, *which extensions this forest's chain claims and
who claims each*: `supported_formats(config, extra, registry)` returns one
entry per extension in the same precedence order `discover_converters`
applies, naming the first claimant as `hook` (a G.6 command hook),
`ext:<id>` (a Part L extension), `describer` (a host-injected extra),
`plugin:<name>` (an entry point) or `builtin`. Every surface that tells a
person what may be ingested MUST read this answer and MUST NOT keep a list
of its own — a copied list is what let a `.pdf` stay greyed out in a
console while the converter for it was installed, enabled and loaded.
...
snapshot is a **container**: `<forest>-<stamp>.forest`, a compressed
archive holding
...
- `payloads/<path>` — the BONE tier (A.3.1) at its forest-relative path;
...
7. **A tool description names the neighbour that does what it refuses
   ...
   that grows past what a session should pay is rule 1's concern. `answer` (v0.82) says that a `media:<id>`
   reference in its reply, or a `sources[]` item of type `media`, is
   opened with `view` — a reference rather than a refusal, and the same
   rule (J.10.9).

8. **The SDK's refusal wears the envelope (v0.82).** The SDK validates a
   tool's arguments against the input schema before the tool runs, and
   answers a mismatch as its own prose under `isError` — so a consumer
   that had learnt to read C.12's envelope met a second failure language
   on the same surface and wrote a text parser for it. Rule 2 made the two
   families agree on the FLAG; this rule makes them agree on the BODY. A
   call the SDK refuses — arguments the schema rejects, a tool that does
   not exist — is re-stated as the C.12 envelope: `E_SCHEMA` naming the
   tool and the parameter, with the message the signature table
   (`signatures.py`) produces for the same arguments, so the sentence is
   REST's own; where the table accepts what the schema refused, the
   parameters the schema reported are named instead. An unknown tool is
   `E_NOT_FOUND` naming it, as REST answers an unknown primitive, with the
   served names in the hint. A crash inside a tool stays the SDK's generic
   message, re-stated as `E_INTERNAL` naming the tool and nothing else
   (C.12's last resort). The SDK still decides, as it does J.1.1's 421 and
   J.1.4's 404: the host rewrites the sentence and never the verdict, and
   `isError` stays set (rule 2).

**F.210 (acceptance).** `answer` with `k: "many"` over MCP is `isError`
with a body `{error: {code: "E_SCHEMA", message}}` naming `k`, the message
equal to REST's for the same arguments; `view` without `id` names `id`;
`tools/call` of a tool that does not exist is `E_NOT_FOUND` naming it; the
flag and the envelope agree on every one. Covered by tests.
...
- **The mask only narrows.** The key's effective authority is the
  ...
  it.
- **`caps` ⊆ `{read, ingest, answer}`, default all three (v0.82: J.2.7
  rule 4; `{read, ingest}` before it).** A pair key exists to
...
outside `{read, ingest, answer}` is `E_SCHEMA`. All covered by tests.

#### J.2.7 The question is a capability (v0.82)

`answer` (J.10.3) was gated on `read` plus a model binding. The capability
said who may see the material and the binding said which model reads it,
and nothing said who may make the forest's model run: every reader could,
on every forest with a binding, and neither a grant, a key nor the MCP menu
recorded that fact anywhere. A deployment holding an expensive client model
beside a cheap forest-bound one wants exactly that decision — this key asks
the forest, that key reads it — and had no place to write it down. From the
other side, a consumer whose client filtered the tool out reported that the
product had no way to ask, and nothing here could contradict them: the menu
was the same for every key, so the menu was evidence of nothing.

Normative:

1. **`answer` is the seventh capability token.** `CAPS` is `{read, write,
   query, tend, ingest, admin, answer}`. It is granted per (principal,
   forest) like the others, appears in a J.2.6 mask like the others, and
   `admin` implies it as `admin` implies every other. It gates the
   `answer` composite (J.10.3) on REST and MCP alike, in place of `read`;
   the refusal is `E_FORBIDDEN` naming `answer`, decided BEFORE the
   binding is looked at — a key that may not ask is told so, not told
   which model it may not use. `curate`, `harvest` and every primitive
   are unchanged.
2. **The binding is still required.** The capability says who may ask;
   the binding says what answers. A granted key on a forest without a
   binding receives the J.10.3 refusal, unchanged.
3. **Nobody's behaviour changes without a decision.** A registry created
   before this version holds grants and masks that carried `read` and
   meant `answer` by it. On first open under this version a one-time data
   repair — the `DATA_REPAIRS` mechanism J.10.8's amendment introduced,
   stamped in `PRAGMA user_version` — adds `answer` to every grant that
   carries `read` and to every non-null mask that carries `read`. It runs
   once: a grant an operator narrows to `read` afterwards stays narrowed,
   and only the stamp tells the two apart. The owner (J.2.4) holds every
   token and has no row to repair.
4. **Every door that minted `read` alone mints `read` and `answer`.** The
   J.2.6 pair ceiling is `{read, ingest, answer}` and the default mask is
   all three; `station key --caps` defaults to `read,answer`; the People
   console's `reader` level carries both, as does every level above it. A
   door that kept minting `read` alone would hand out a narrower
   credential than the same door minted the day before, which is a
   decision nobody made.
5. **The menu says it.** The MCP `answer` tool is listed by `tools/list`
   only for a key that holds `answer`, through its mask, on at least one
   forest it reaches — L.7 rule 2's reasoning: a tool that can only refuse
   costs every session its description and teaches nothing. A forest that
   grants it and has no binding still lists it: the binding's absence is
   an operator's unfinished setup and the refusal names the repair,
   whereas a missing grant is a decision. `forests()` (J.1.2 rule 6) and
   `/v1/me` report the token among `caps`, masked, so an agent and a
   console read the same fact. The `instructions` (J.1.2 rule 5) keep
   naming `answer` — they describe the surface — and say it appears for a
   key granted it.
6. **The consoles follow the token.** The Ask console (J.5.1) is reached
   with `answer`, not `read`; the Skills console (J.5.12) teaches the
   `answer` call only for a key holding it, and otherwise names the
   capability in one line; the integration manual (J.5.11) lists the tool
   under its own token; the People console offers it in the fine-tune
   list and in the levels table.

*(Informative.)* Why a token and not a per-forest switch: a switch says
whether the forest answers, a token says to whom — and the case that
motivated this is one deployment holding both an expensive client that
should navigate and a cheap one that should ask. C.14 rode `prune` on
`write` because "a fourth token would invalidate every issued key"; that
cost is paid here by rule 3, and it is paid because the token is the point.

**F.206 (acceptance).** A grant of `{read}` alone: `POST .../answer`
answers 403 `E_FORBIDDEN` naming `answer` and no provider is called; the
MCP `tools/list` for that key omits `answer` and names every other tool;
`forests()` and `/v1/me` list no `answer`. The same grant plus `answer` on
a forest without a binding answers the J.10.3 binding refusal. A registry
holding a grant `read,query` and a masked key `["read","ingest"]` from
before this version opens with `answer` in both, `user_version` advanced,
and a grant narrowed to `read` afterwards stays `read` across a reopen.
`POST /v1/auth/pair` accepts `caps: ["answer"]`, still refuses
`["write"]`, and its default mask carries `answer`. The owner's `/v1/me`
lists it. Covered by tests; the console's role presets, capability list
and Ask gate by a static check.
...
caps:      [read, write, query, tend, ingest, admin, answer]
...
| `answer` (J.10.3) | requires `answer` cap (J.2.7, v0.82); the retrieval underneath inherits every rule above, so a bound model reads nothing the key could not |
...
The row of J.4 records who read what. Four groups of fields are added to it
(three in v0.73, `via` in v0.80), each the answer to a question an operator
arrives with, and none of them content:
...
3. **`via` who acted on the principal's behalf (v0.80).** `ext:<id>`
   when an extension was in the path, absent otherwise. `principal` keeps
   its meaning to the byte — whoever *caused* the act — because the question
   an access log exists to answer is who asked for this, and an extension is
   not an answer to it. Without this field an extension's model spend is
   attributed to the person who triggered the ingest, which is the same
   class of quiet misattribution the other three groups were added to end.
4. **`ms` and `model_ms` what it took.** `ms` is the engine's own span,
...
default landing console for a principal holding `answer` (J.2.7, v0.82;
`read` before it); a principal without it lands on the first console
they do have.
...
- **It teaches bytes (v0.77).** The saving block MUST show an upload
  entry carrying `b64` for a file that is not text, say what an image or
  audio file becomes (a `type: media` node whose bytes `view` serves),
  say that `plant` carries no bytes, and say that `origin` is a pointer
  for people that the forest never follows. An agent taught only `text`
  concluded the product had no way to store an image (v0.77's
  changelog), and every fact it lacked was one sentence long.
- **It teaches the passport (v0.78).** The saving block MUST show an
  upload entry carrying `passport` beside `b64`, say that it is planted
  as the node's own scent in the same commit, that such an entry is never
  sent to the curation model, and that a link is `related-to` only and
  dropped when it fails the check.
...
5. **The trail is the forest's own structure, drawn as two journeys
   (v0.72).** The chain from the root to a hit follows `parent`, which J.11
   has already filtered to what this principal may see: a chain that leaves
   the scope stops, and never draws a segment to a node the caller cannot
   read. Shared ancestors are drawn once, at the earliest stage that needed
   ...
   the whole chain says the agent arrived, in one move, on the exact file it
   needed, and that is the claim an entry search does not make: the value of
   dropping somebody NEAR is lost the moment the picture shows them landing
   ON. Because the split is decided by the destination and not by a
   threshold, the flight cannot reach a document by accident.

   The walk is the agent's **own movement**, and the colour means that in
   both modes so that one legend line can describe it:

   - on a **sweep**, one step: the branch into the file that was opened;
   - on a **walk**, the real hop sequence. A hop that names ONE node
     (`move`, `pick`, `look`) is a position. A hop that returned a SET
     (`locate`, `sniff`, `scan`) is the agent looking around from where it
     already stands, and a line drawn to the first of ten results would
     invent a step nobody took. Consecutive hops on one node collapse:
     reading a node and then picking it is one place, twice.

   A sweep therefore has a flight and one step, and never a hop sequence —
   an absent line is the true statement that no walking occurred.

   **Every node the trail touches is named**, not only the nodes it stopped
   at. A caption on a way-through branch is written more quietly than one on
   a result and yields to it when the map is crowded — losing an ancestor's
   name is a cost, losing a result's is a defect — but a route whose
   waypoints are anonymous is a shape, and the branch an answer climbed
   through is half of where it went. The captions MAY be withheld entirely
   above a density at which they would cover the map; that ceiling is
   implementation freedom and the count that decides it is the marked set,
   which is what says how busy the answer was.

6. **A trail is not a shortcut, and the two journeys are two colours
   (v0.72).** Each line carries its own colour token, and a
   `discovered-shortcut` carries a third: a shortcut is a fact the forest
   holds, a flight is where one question was dropped, a walk is where it
   then went, and three different facts MUST NOT arrive in one colour.
   Neither line's colour may be one the dots use, since those are coloured
   by home branch (rule 9). Proposals (`confidence < 1`) stay dashed, as in
   J.5.4 — and under rule 9 this panel draws none of them, which satisfies
   the distinction by not raising it.
...
9. **The background is dots, and the trail is the only line (v0.67; two
   lines, both moving, v0.72).** Rule 5's two journeys are the trail; no
   other line is drawn. **Both MUST move** — a still line beside a moving
   one reads as a wire beside a route, and both of these are journeys — and
   **every leg MUST advance on one clock**, never staggered by depth or by
   rank. Several hits are several drops leaving the base at once; revealing
   them one after another invents an order the retrieval never had, since an
   entry search returns a ranked set and not a sequence of departures. The
   walk may begin after the flight, because that order is real. The
...
#### J.5.16 The Audit console (v0.73)

The console shows the rows of J.4 and it MUST answer the two questions an
operator opens it holding — *is anything wrong* and *what is this costing
me* — before it shows any of them. A table alone answers "what happened at
14:32", which is the question somebody asks second.

1. **The screen leads with the set, not with the newest row.** How many
   calls, how many were refused, what was spent, and what the answer store
   avoided spending. Four numbers, above the table.
2. **Those numbers are the host's `totals` (J.4.3), never a sum the console
   computed over the rows it fetched.** This is J.10.6's rule about
   instruments applied to a count: a console that adds up its own page is
   describing its page size.
3. **A store hit's cost is presented as avoided and MUST NOT be added into
   the spend.** The two are different facts about money and a single
   "cost" figure covering both is a bill nobody can reconcile.
4. **An unpriced call is shown as unpriced, never as free.** A local
   provider publishes no catalogue; rendering its calls at `$0.00` invents
   a price the deployment never had.
5. **Every filter rides the address** (J.5.8), so a filtered view is a link
   somebody can send to the person who has to look at it. Restoring one
   spends a read and nothing else.
6. **A refusal is shown as its code**, beside the call. The console MUST
   NOT invent a sentence for a code it does not know: a Station newer than
   the console renders the code itself rather than nothing, in the same
   spirit as A.2's declared vocabulary.
7. **The argument digest is shown.** It is the only field in the row that
   says WHICH node was read, it is already digested by J.4 and already in
   the response; withholding it in the console makes nothing more private
   and makes the log unreadable. It is forest data and is rendered as
   stored (J.5.3).
8. **Where a filter offers values, they are the ones the response says
   occur** (J.4.3), not a list the console carries. A console-side list of
   primitives goes stale the release after it is written, and it goes stale
   silently — the filter simply stops offering the call that was added.

The console remains what J.5 says every console is: a reader of the
documented route. It gains no aggregate of its own, and a principal who
cannot read a forest's rows over the API cannot see them counted here.

#### J.5.17 The operator's hands: prune and transplant (v0.75)

`prune` has been contracted since v0.56 and `transplant` since v0.58. Both
are dispatched by the Station, audited, evented, and taught to agents by
the skill Studio itself generates — and neither has ever been reachable
from Studio. The operator who plants a node in the wrong place, or ingests
a file they should not have, holds strictly fewer powers over their own
forest than the agent they connected to it.

Normative:

1. **Both are offered wherever a node is the subject** — the Read console
   and Explore alike — gated on `write` exactly as the editor already is
   (J.5.4), and absent rather than disabled when the grant does not carry
   it.
2. **A prune states what it is about to do, in the engine's own words.**
   The confirmation MUST name the node by `title` AND `id`, and MUST say
   the two things C.14 makes true and a user will not assume: the passport
   is removed **through git, so the history keeps it**, and a local
   payload is **moved to `_derived/graveyard/`, not deleted**. A delete
   dialog that overstates the damage teaches the operator to fear a
   reversible act; understating it is worse.
3. **`E_ANCHORED` is rendered as the list it carries, never as a failure.**
   C.14 refuses a node with backlinks and puts `anchors` and
   `anchor_count` on the envelope. The console MUST show those anchors as
   navigable nodes — that refusal is the most useful thing this screen
   ever says — and offer `force` as a SECOND, separate decision that
   restates what it does (the backlinks are stripped in the same commit).
   Where an anchor is out of the caller's scope C.14 gives a count and no
   names; the console MUST show the count as a count and MUST NOT imply
   the list it received is complete.
4. **A transplant is a rename with a waymark, and it is presented as
   one.** The new id is composed the way J.5.7 composes a plant — the
   parent is chosen, never typed — and the confirmation MUST say that the
   old address keeps answering (`E_MOVED` carrying `moved_to`, C.15) so
   nothing already written down is broken. Leaf only: a branch offers no
   transplant control at all, rather than a control that refuses.
5. **The console never invents a bulk delete.** `prune` is one node per
   call by contract, each with its own commit and its own anchor check;
   a loop over a selection would let one `E_ANCHORED` strand a
   half-finished sweep with nothing recording where it stopped. If bulk
   removal is ever wanted it needs its own contract, not a for-loop in a
   browser.
6. **Both refresh what the address points at.** A pruned node's id no
   longer resolves, and a transplanted one has moved, so the console MUST
   move the selection out of the query string (J.5.8) instead of leaving
   an address that reloads into an error.

#### J.5.18 Tags are edited, applied and browsed (v0.75)
...
and Studio surfaces them as a comma-separated text field on a screen
reachable from exactly one console.

Normative:

1. **The scent is editable wherever a node is read.** Title, summary and
   tags are the fields a reader is best placed to fix, precisely because
   they are reading; confining the editor to Explore put the repair on a
   different screen from the moment the need for it is noticed. The Read
   console MUST offer them, through the same `graft` (J.5.4) — one door
   for humans and agents stays the rule, and this adds a doorway, not a
   second door.
2. **Tags are applied in bulk, and the bulk write is stated.** A
   selection of nodes MAY have tags added or removed in one action.
   Because there is no batch `graft`, this is N calls: the console MUST
   show progress against the total, MUST report per-node failures without
   abandoning the rest, and MUST NOT claim completion for a partial run.
   The operator is authoring N commits and is told so.
3. **Adding is not replacing.** A bulk apply UNIONS the tags into each
   node and a bulk remove subtracts them; neither overwrites a node's
   existing set. One careless action must not be able to erase curation
   across a selection — the same instinct G.2.6 rule 3 applies to derived
   aliases.
4. **A forest's tags are browsable, with counts.** The vocabulary is
   free (A.3), so it drifts: `invoice` and `invoices`, `q1-2026` and
   `2026-q1`. A list of the tags that actually occur, with the number of
   nodes each carries, filtered by the caller's own scope, is what makes
   that drift visible and is what turns a tag into a way in — clicking
   one filters the forest by it. Counted over the whole scope in SQL and
   never over a page, for J.4.3's reason: a total computed from what is on
   screen changes when somebody changes the page size. Served by
   `GET /v1/forests/{f}/tags` under the `read` capability, scope-filtered
   in SQL (`Policy.sql_scope`, C.13.3's rule), ordered by count then name,
   and clipped at a stated cap with `truncated: true` — never in silence
   (C.6.2's pattern).
5. **Nothing here bypasses G.4.2's validation.** A tag typed by a person
   is validated exactly as one written by the Curator, and a refusal names
   the tag and the rule. The console MUST NOT quietly normalise a tag into
   something acceptable: silently rewriting what somebody typed is the
   same failure as silently dropping what the model wrote.

**F.167 (acceptance).** The operator's hands work and refuse the same way
the API does. From the console, pruning a leaf removes it, keeps its
history in git, and moves its payload to `_derived/graveyard/`; pruning a
node with backlinks renders the `anchors` C.14 returned as navigable
nodes rather than an error string, and `force` is a second, separately
confirmed decision. An anchor out of scope shows as a count and the list
is not presented as complete. A branch offers no transplant control at
all. After either act the selection leaves the query string, so a reload
does not land on a node that is gone.

**F.168 (acceptance).** Tags are correctable where they are read. The Read
console edits title, summary and tags through `graft`. A bulk apply over a
selection UNIONS its tags into every node and leaves each node's existing
tags in place; a bulk remove subtracts only what was named; a run where
one node refuses reports that node and still completes the others, and
never reports completion for a partial run. The tag list is computed over
the caller's whole scope in SQL — changing the page size does not change a
count — and a tag typed by a person is validated by G.4.2's rule, with the
refusal naming the tag rather than rewriting it.

**F.169 (acceptance).** The change is measured, not asserted. `locate`
quality is reported over `bench/questions-locate-v1.json` before and after
the curator change, on a forest re-ingested with each — recall@1 and MRR,
broken out by the set's four classes and by its silence questions. A
regression in any class is NAMED in the result rather than averaged into
the total, and the silence questions MUST NOT get better by returning more
results. A change measured only on a saturated set has not been measured
(v0.70's rule, and the reason that set exists).

**F.170 (acceptance).** The timeline is a window. The graph mode's docked
timeline carries two controls on one scale — a start and an end, each
named for assistive technology — and the start defaults to the origin, so
the picture opens as v0.38 left it. A node outside the window is not
painted, not hit-tested and not framed by the camera, and IS still stepped
by the simulation, so the layout under a narrow window is the layout of
the whole forest. The readout names both days and the count inside the
window over the total; "now" restores the whole forest. The window enters
neither the persisted view settings nor the address. Read off the source
on F.137's boundary (the canvas is unverifiable), each check verified to
fail with the v0.75 behaviour put back.

**F.171 (acceptance).** The path is named where it is read. An MCP
`ingest` of a base64 image lands a `type: media` node whose `look`
carries `payload_type` and a `payload_bytes` equal to the upload, whose
`view` returns those bytes, and whose `prune` moves them to the
graveyard — one test, end to end. `plant` of a `type: media` node
without `payload` is `E_SCHEMA` whose hint names `ingest` and `b64`;
with a payload naming no file, or one outside the forest, `E_SCHEMA` at
the plant; the Gardener's own adopt under `archive: never` still plants
and `look` says `payload_missing` for it. A media node whose bytes are
gone reads `payload_missing: true` from `look` while `view` answers the
unchanged envelope. `sniff(scope: "_meta")` is `E_SCHEMA` naming the
dialect and an unknown scope is `E_NOT_FOUND` naming the scope sent.
The served tool descriptions name their neighbours (J.1.2 rule 7) and
the instructions name `b64`; the generated skill's saving block does
too.

#### J.5.19 The question has a period (v0.79)

An operator asking for "the two screenshots I uploaded today" is asking a
bounded question, and C.13.1 has carried the bound on `answer` since v0.52
— `since`, `until`, `date_field`, forced onto every hop of a walk (rule 7)
and keyed (rule 8). The Ask console could not send it. Every window an
operator had in mind was therefore the model's to guess, and J.10.5 rule 3
says why the guess had nothing to go on.

- **The console offers the window.** Two date inputs beside the question,
  `since` and `until`, sent as C.13.1's own parameters on `answer` when
  set and not sent otherwise — a call without them is byte-identical to
  the console's before this version. `date_field` stays `created`: the
  console asks when material arrived, which is the question a fed forest
  asks (J.5.4's window, one console over).
- **It rides the address beside `q`, and never a preference.** The window
  is part of the question, not a taste: a shared link asks the same
  bounded question. It is read from the address at mount exactly as `q`
  is (J.5.8: the address restores a page, never a call — nothing fires)
  and typing writes it back nowhere. It MUST NOT be kept as a browser
  preference (`monkeyllm.ask.prefs` or any other): a window remembered
  from yesterday opens tomorrow's forest empty, which is v0.76's lesson in
  J.5.4 applied here.
- **The answer is labelled by the echo, not the inputs.** What the console
  shows beside the answer is the response's `window` (C.13.1 rule 6), the
  normalised dates the search actually used — never the strings in the
  inputs, which may be a month the engine expanded. The controls are date
  fields, and a date field shows nothing for a coarser bound a link
  carries (`?since=2026-08`): the bound is still sent as it arrived, the
  engine expands it (C.13.1), and the echo is what tells the operator —
  the console MUST NOT expand it itself, because a second expansion is a
  second reading of C.13.1 that agrees with the first only where somebody
  compared them. The run history (J.5.9) badges a bounded run, so two runs
  of one question with different windows read as two runs, and restoring
  a run restores its window with it: that is the record, not a preference.
- **A bad bound is the engine's refusal.** An unparseable date is C.13.1
  rule 4's `E_SCHEMA`, rendered as any other refusal; the console does not
  pre-validate into a message of its own.
- The cURL the console offers carries the window, because it carries
  everything else the form would send.
...
#### J.8.5 The console accepts what the forest converts (v0.83)

`GET /v1/forests/{forest}/ingest` — the status beside the POST — MUST carry
`formats`: the list G.2's `supported_formats` answers for that forest,
computed through the same discovery the next ingest will run (the forest's
command hooks, the extensions it enables, the describer the host injects,
the plugins and the built-ins), each entry `{extension, via}`.

A console MUST derive from it the file picker's accepted types, the filter
applied to a dropped folder, and the sentence that says what may be
dropped; it MUST NOT carry a list of formats of its own. A file refused by
type MUST be refused **by the console, before upload**, naming the formats
the forest takes, so that "no converter for this format" is a fact about
this forest and not about a constant in a build. An entry whose extension
is not one of the built-in text formats travels as `{name, b64}`; the text
shape stays for text.

The rule exists because the alternative shipped: the v0.82 console listed
the built-in formats in its source, an operator installed and enabled a
PDF converter, and the picker greyed out every `.pdf` while the report
would have accepted it. Nothing had failed. The surface had simply never
been told what the mechanism under it changed.
...
#### J.8.4 The passport travels with the bytes (v0.78)

v0.77 named the path bytes take into a forest. The first agent to walk it
found the next gap: it had *seen* the screenshot it was uploading, knew
what the picture showed and which nodes it belonged beside, and the only
place that knowledge could go was a second call after the plant. Between
the two calls the node existed with a stub summary — a scent no `locate`
finds — and a batch of twenty screenshots was forty calls and forty
commits. A media node's worth is its bytes; its *findability* is its
passport, and the caller who holds the bytes very often holds the passport
too.

An `upload` entry MAY therefore carry `passport`:

```json
{"name": "print-templates.jpg", "b64": "…", "source_url": "https://…",
 "passport": {"title": "Templates library — first screen",
              "summary": "The templates library as shipped: filter bar, three cards per row, empty state copy.",
              "tags": ["findleads", "templates", "screenshot"],
              "aliases": ["templates library print"],
              "links": [{"target": "tasks/findleads/front-end/031-templates", "note": "the screen this task describes"}],
              "notes": "Taken on the staging tenant; the third card is a placeholder."}}
```

Normative rules:

- **Shape is checked before the first byte stages.** `passport` MUST be an
  object whose keys are a subset of `title, summary, tags, aliases, links,
  notes`; an unknown key, a non-string title or summary, a summary that
  cannot be fitted to the A.4 budget (empty, boilerplate), a non-list
  `tags`/`aliases`/`links`, more than 16 aliases or one over 80
  characters, a link without a string `target` or with a `rel` other than
  `related-to`, or `notes` over 4,000 characters is `E_SCHEMA` naming the
  entry and the field — and, as for `source_url`, a batch with one bad
  entry stages **nothing**.
- **The passport is applied where a reviewed draft is applied** (J.8.1): as
  an `on_curate` hook, so converter, content policy (G.7), plant and commit
  are exactly those of any adopted file. The Station adds no second way to
  plant from a passport.
- **The same rules as a reviewed draft, because it is one.** `summary` is
  fitted to A.4; `tags` re-cleaned and capped at the G.4 maximum; each link
  re-checked against G.4.2.1 — `related-to` only, target existing and in
  the caller's scope, never a branch, never the node itself or its parent,
  at most three, pinned at confidence 0.3, a `note` clipped to its budget.
  A link that fails the check is dropped, not fatal: the node still lands,
  and `look` shows what survived. `aliases` join the ones G.2.6 derived
  from the file name — the file's own name still counts — first come first
  kept, under the C.8 cap of 16. `title` replaces the converter's. **And
  every refusal is counted**: a tag the rule refuses and an alias the cap
  clips are counted in the report's `tags_dropped` and `aliases_clipped`
  exactly as the model's are (G.4.2 rule 1). A passport is not a way to
  lose a tag in silence.
- **`notes` is the person's section.** It is appended as the node's
  `## Notes` (C.2.1), which `look` carries for a dataset and — as of this
  version — for a media node (C.2.1 rule 2), and which the sweep's
  material and the walk's entry carry for both (C.2.1 rule 6), so what
  the uploader knew about the bytes travels with every read of them.
- **An entry with a passport is never sent to the curation model.** The
  caller declared knowing more than the describer would guess (G.5.1), and
  a model's answer would ship instead of what was declared (J.8.1's own
  rule). Entries **without** a passport in the same batch keep the bound
  curator: the decision is per draft, and the report's curation counters
  count only what the model was asked. The describer (G.5.1) is a
  converter, not the curator, and still runs where a `vision` role is
  bound: the body is its description, and a passport carries no body.
- **Refreshes are unaffected, and said so.** Curation never runs on
  `sync` (G.3), and a passport is curation: bytes re-sent for a node that
  already exists are refreshed and their passport is NOT applied — a
  `graft` is how scent somebody may have edited since is changed, never a
  silent rewrite. The report MUST name every such entry in
  `passports_ignored` (empty when every passport was applied): an
  unapplied passport that nothing mentions is the silent drop this
  document forbids.
- **Nothing else moves.** `adopt` and `sync` do not take passports (a
  directory has no caller who knows each file); `origin` stays a pointer
  (A.3); `view` and `plant` are byte-identical to v0.77.

**F.172 (acceptance).** Over the MCP surface an agent actually holds:
`ingest(mode: "upload")` of a base64 image with a passport plants a
`media` node whose `look` carries the declared title, summary, tags and
aliases, a `related-to` edge to the named existing node, and whose body
holds the declared `## Notes` — in one job and one commit, `payload_type`
and `payload_bytes` present. A batch mixing one entry with a passport and
one without plants both, the second with the derived summary. A passport
with an unknown field, a boilerplate summary or a `rel` other than
`related-to` is `E_SCHEMA` **and the staging area holds no file** from
that batch. A link to a branch or to an absent node is dropped, the node
still lands. A passport whose tags the rule refuses lands with the
survivors and the report's `tags_dropped` counts the rest; aliases past
the cap count in `aliases_clipped`. Bytes re-sent for an existing node
with a new passport are refreshed, the old scent stands, and the report
names the entry in `passports_ignored`. `look` on the planted media node
carries the declared `notes`, and so does the sweep's item for it. All
covered by tests.
...
- **`answer(question, k, terms?)`** runs the scoped `harvest`, hands the
  ...
  model_ms, evidence, harvest, trace}` (J.10.4). Requires the `answer`
  capability (J.2.7, v0.82; `read` before it).
...
- **Read-only, by whitelist, not by capability.** The loop offers `locate`,
  `sniff`, `look`, `move`, `pick`, `scan`, `query`, `coverage` (v0.67),
  `calendar` (v0.79) and nothing else. The policy would already refuse a write to a principal
  ...
  **`calendar` is on the list for the same reason (v0.79).** A question
  with a period in it — today, this week, since the last release — is
  decided by where the material sits in time, and C.13.3 is the read built
  for that: periods and counts off the catalog, no body opened, every
  count the calling policy's own (C.13.3's scope rule). Without it a walk
  asked for "today" could only guess a date, or read the whole forest to
  find one. Where the caller bounded the hunt (C.13.1 rule 7) the bound is
  forced onto `calendar` as onto every searching call, so the map the
  model sees is the map of what it may reach.

  **The menu names what the tools take (v0.79).** The loop forwards the
  model's arguments to `ScopedVine` as they are, so a parameter the wire
  accepts was always a parameter a walk could use: `since`/`until`/
  `date_field` on `locate`, `sniff` and `scan` (C.13.1), `type_filter` on
  `locate` and `sniff`, `filter: {type, …}` and `recursive` on `scan`,
  `granularity` on `calendar`. The menu the loop offers MUST name them, by
  the sentence above applied one level down: a parameter the model is
  never told about is a parameter the whitelist did not admit. A walk that
  needed exactly these read the root, scanned it flat (no child matched),
  grepped every body for the word "media" and answered that the forest
  holds no pictures, while `coverage` had counted two in its first hop.
...
**A hop names the argument that decided it (v0.79).** The record rendered
`query`, `terms`, `sql`, `section`, `rel`, `scope`, `parent_id`,
`direction` and `k`, and none of `filter`, `type_filter`, `since`, `until`,
`date_field`, `recursive` or `granularity` — the arguments that decide
whether a listing is empty. `scan _index → 0 node(s)` on a console then
reads as an empty root, when it was a flat scan with a type filter over
five branches. The record MUST carry every argument from that list the
model set, clipped like its other string fields; an argument the model did
not set is absent, never a default written in. A record written before this
version lacks them, and a consumer MUST read the absent field as *not set*
— never as a default, and never inferred from `out`.
...
**Four things the loop's prompt MUST say (v0.67; rules 3 and 4, v0.79).** How a prompt is worded is
...
3. **What day it is.** The prompt MUST state the host's date, and the
   host's clock is the one that stamps `created` on a node this host
   plants (the same `date.today()`), so "today" in the question and
   "today" in the passports are one day. Without it a question with a
   relative period — today, this week, last month — is unanswerable by a
   model holding every window parameter there is: it can write `since`
   and has nothing to write in it. The prompt is not on J.10.7's key, so
   the date enters the walk's key explicitly (J.10.7, v0.79). Where the
   caller bounded the hunt the prompt states the bound as before (C.13.1
   rule 7) and states that the bound replaces any window the model
   authors.
4. **A kind of node is found by its filter.** The prompt MUST route a
   question about a kind of node — the pictures, the datasets, the
   decisions — to `type_filter` on `locate`/`sniff` and `filter: {type}`
   on `scan` (with `recursive` where the kind may sit anywhere), and MUST
   say that a `sniff` for the type's name is not that: it greps bodies for
   a word and returns every document that mentions it, which is how a
   walk for two pictures came back with four task documents.
...
**F.173 (acceptance).** The walk's whitelist includes `calendar`, and a
scripted walk that calls it receives C.13.3's buckets through
`ScopedVine`. The system prompt the loop serves names `since`, `until` and
`date_field` for `locate`/`sniff`/`scan`, `type_filter` for
`locate`/`sniff`, `filter` with `type` and `recursive` for `scan`,
`calendar` with `granularity`, and the host's date as `date.today()`
formats it. A model-authored `since` on a hop reaches the engine (the
hop's result set is the windowed one, and its `ids` say so); where the
caller bounded the hunt, the caller's `since`/`until` replace the model's
on `locate`, `sniff`, `scan` and `calendar`, while `coverage` is left
unbounded. The hop record carries `filter`, `type_filter`, `since`,
`until`, `date_field`, `recursive` and `granularity` when the model set
them and none of them otherwise. The walk's store key differs across two
dates and the sweep's key takes no date. The Ask console (J.5.19) offers
`since`/`until`, sends them as C.13.1's parameters on `answer`, prefills
them from the address, and writes them to no browser preference. All
covered by tests; the console by a static source check on F.137's
boundary.

**The walk is on both surfaces (v0.82).** The MCP `answer` tool was
registered without `hops`. No section said the walk was REST's: J.10.11
notes it stays lane-bound and is opt-in per call, and that sentence was
read as a reason to keep it off a menu it was never about. A consumer found
the walk on REST and built a transport of their own to reach it, which is
J.1.2's failure — a tool the menu does not carry is a tool the client does
not have — arriving from the other side. Normative:

1. The MCP `answer` tool carries `hops` (`true`, or an integer budget),
   forwarded verbatim; `terms` beside `hops` is `E_SCHEMA` exactly as on
   REST (J.10.3), and the response is the walk's record as REST serves it.
   The walk stays lane-bound (J.10.11); nothing about its execution
   changes with the transport.
2. **A hop is progress.** A walk runs for as long as its hops take — past
   a minute is ordinary — and an MCP client ends a call on its own clock.
   When the request carried a `progressToken` (the protocol's own opt-in),
   the Station MUST report one progress notification per completed hop:
   `progress` the hop's `n`, `total` the budget, `message` naming the tool
   and its `out`. It rides the observer J.10.12 already feeds — the hop
   record just appended, never a second rendering — and it obeys J.10.12
   rule 4: emission never blocks the lane, and a notification that fails
   never fails the hunt. Without a token nothing is sent, and a call
   without `hops` reports nothing.
3. The tool's description MUST say what a hop costs (one model call) and
   that a walk may outlast a client's timeout: the parameter that decides
   how long the call lasts is not one to discover by experiment.

**F.207 (acceptance).** `answer` over MCP with `hops: 2` against a stubbed
model that searches once and answers returns `hops` and `read` equal, field
for field, to the REST response for the same call (timings excepted);
`terms` beside `hops` answers the `E_SCHEMA` envelope with `isError` set; a
walk reports one progress notification per hop through the request's own
context before its result, `progress` equal to the hop's `n` and `total`
the budget, and a sweep reports none — the SDK is what turns a report into
`notifications/progress` when the request carried a `progressToken` and
into nothing when it did not, and that half is the SDK's own contract. The
served description names the cost and the duration. Covered by tests.
...
**The reasoning is not the reply (v0.82).** A binding's `reasoning` is
`off` by default and the OpenRouter flag says so on the wire; a model
served elsewhere thinks anyway, and a server that does not lift the
thinking into a field of its own leaves it at the head of `content` as a
`<think>` block. That block reached the reader as the answer, and it spent
the budget the prompt had stated for the reply — J.10.8's cut, caused by
text nobody asked for. Normative: a leading `<think>…</think>` (or
`<thinking>…</thinking>`) block is removed from the content at the one
place it is read, before the sweep's text is taken as the reply and before
a walk's turn is parsed as a tool call; the response then carries
`reasoning_stripped: true`, in the record and in the store (a reply with
its thinking removed is a whole reply). A block the cap cut open is
stripped whole — it was all thinking — and the empty reply carries
`truncated` with the provider's reason, which is this section's own case.
A provider that returns reasoning in a field of its own is untouched: that
field was never read. The flag is the operator's signal: a model that
thinks with reasoning `off` needs the bump `on` buys, and the flag is how
a console says so.

**F.209 (acceptance).** A stubbed provider answering
`<think>x</think>Answer` yields `answer: "Answer"` and
`reasoning_stripped: true`; answering `Answer` yields a response with no
such field, byte-identical to before; a walk whose turn is
`<think>…</think>{"tool": …}` parses and runs the tool; an unclosed
`<think>` with `finish_reason: "length"` yields an empty reply flagged
`truncated`. Covered by tests.
...
**Over MCP the reference is a call (v0.82).** The console resolves
`media:` through J.14 with the viewer's credential; an MCP client has no
J.14 and no resolver, so a reply that embedded an image handed its reader
an address that opens nowhere, and `sources[].type` was the only hint that
an image existed. The bytes do not ride the response — C.6d rule 5 stands,
a model's context receives pixels by that model's own choice — and the
choice is named: the `answer` tool's description, the `instructions`
(J.1.2 rule 5) and the generated skill (J.5.12) MUST say that a
`![…](media:<id>)` in the reply, or a `sources[]` item of type `media`, is
opened with `view(forest, id)` when the client's model can see images.
J.1.2 rule 7, applied to a reference instead of a refusal.

**F.211 (acceptance).** The served `answer` description and the
`instructions` name `view` beside `media:`; the generated skill's core
does, for a key holding `answer`. Covered by tests and the skill's static
check.
...
7. **The MCP surface gains nothing.** No tool, no subscription, no
   ...
   *(Amended v0.82.)* The per-hop progress a walk reports over MCP
   (J.10.5) is not a channel in this section's sense: it is the
   protocol's own progress notification, sent only when the client asked
   for it on the request, carrying a hop's `n`, the budget and a message
   — never the record. The REST channel stays the console's, and nothing
   is announced.
...
### J.10.13 The answer at the size the caller can carry (v0.82)

An `answer` response carried the whole of its retrieval: the sweep's
`harvest` with every excerpt and every match, a walk's `read` with every
body `pick` opened and every row `query` returned, and the Part D `trace`.
Each is right for the console the shape was designed for (J.5.15 draws the
bundle; J.10.4 explains the cost). Over MCP the reader is a model, and the
caller that chose `answer` chose it so that the cheap model bound to the
forest would read the material and its own expensive one would not — and
was then handed the material anyway, in a block billed by the token and
carried forward on every later turn. A consumer measured it and pruned the
response in a proxy, which is C.11's mistake by another road: a size
decided outside the contract.

Normative:

1. **`answer` accepts `detail`**, one of `full`, `sources`, `answer`;
   absent is `full`; any other value is `E_SCHEMA` naming the parameter.
   Both surfaces, from the C.12 signature table.
2. **`full` is byte-identical** to a call without the parameter.
3. **`sources` drops what carries text or machinery** — `harvest`, `read`,
   `turns` and `trace` — and keeps `answer`, `evidence`, `sources[]` (id,
   title, summary, type, trail: the citation block of J.10.4/J.10.9, which
   is what a reader forwards), `hops` (verbs, counts and ids, no text —
   what the walk did, at the size J.10.5 already bounds a record to), and
   the run's own facts: `model`, `model_ms`, `usage`, `cost`, `cached`,
   `reply_tokens`, `truncated`/`finish_reason`, `window`,
   `superseded_excluded`, `exhausted`, `reasoning_stripped`.
4. **`answer` keeps the reply and the ids** — `answer`, `evidence`, and
   the run's own facts — and drops `sources` and `hops` too.
5. **The shape is decided at the close, after everything else.** The
   J.10.7 store holds the whole record and `detail` never enters its key
   (it cannot change what the model writes); the audit row's `size` is the
   record's; the J.10.12 events are the record's; `Server-Timing` is
   unchanged. A hit served at `sources` is the same entry as a miss served
   at `full`.
6. **The J.10.10 refusal is unshaped.** `insufficient_evidence` carries
   its `harvest` at every level: the caller asked to see the evidence
   rather than a paragraph over too little of it, and the refusal IS the
   evidence — there is no reply to make room for.

*(Informative.)* The levels are named for what survives, not for what is
removed, so a client reads its choice off the response it holds. `hops`
survives `sources` because a hop record is the walk's own account of
itself and is bounded by construction (`ids` ≤ 10, `args` the HOP set); a
walk that opened five bodies is five `read` entries of up to 4,000 tokens
each, and that is what the level exists to leave behind.

**F.208 (acceptance).** One question, three calls: `detail: "full"` is
the call without it, field for field (the trace carries clocks, which is
the one thing two calls never share); `sources` carries no `harvest`,
`read`, `turns` or `trace` and the same `answer`, `sources`, `evidence`
and `cost` as `full`; `answer` carries none of those and no `sources` or
`hops`. The second call at any level is `cached: true` off the entry the
first made; the audit rows' `size` is the record's at every level;
`detail: "brief"` is `E_SCHEMA` naming `detail`; a `min_evidence` refusal
under `detail: "answer"` still carries `harvest`. On both surfaces.
Covered by tests.

#### J.10.4.1 The dense layer's two clocks (v0.71)

A hybrid entry search spends outside the forest twice, and the two are not
the same kind of spending.

**Rule 1 — the scan carries `dense_ms`.** A Part D event for a primitive
that ran the Canopy scan MUST carry `dense_ms`: the milliseconds spent on
the vector scan, the rows it pulled back that the lexical half did not, and
the fusion. It is absent when no scan ran, so every BM25-only event is
byte-identical to a pre-v0.71 one.

**Rule 2 — it is NOT `embed_ms`, and neither absorbs the other.**
`embed_ms` is a provider round trip; `dense_ms` is this process. They are
reported apart because they are repaired apart, and because their ratio
inverts: on a cold query the embed dominates (166 ms against 95), on the
same query a second time the memo answers in 0.13 ms and the scan is
everything left. An operator shown one number cannot tell which they have.

**Rule 3 — the host names, the console nets.** `elapsed_ms`,
`retrieval_ms` and `total_ms` keep meaning the whole span, to the byte, and
`trace.dense_ms` is the summed share beside `trace.embed_ms`. A host that
served a subtracted figure would be publishing a number no clock measured.
The panel subtracts both from each row and lists each ONCE at the tail —
the same shape v0.68 gave the embed, for the same reason.

**Rule 4 — the shares are attributed to the call they ran inside**, saved
and restored across nested traced calls exactly as `embed_ms` is, so a
`harvest` does not inherit the share of the `locate` it contains.

**F.149 (acceptance).** With the dense layer on, a `locate` event carries
`dense_ms`, and `elapsed_ms - embed_ms - dense_ms` is the lexical work
alone — on the corpus this was measured against, ~1 ms against a raw span
of 74 to 262. With the layer off, neither field is present and the event is
unchanged.

**F.150 (acceptance).** `trace.dense_ms` is the sum of the steps' shares
and appears only when nonzero; `retrieval_ms` and `total_ms` are unchanged
to the byte by this section. Covered by tests.
...
4. **`admin` on the forest, on the writer lane, and the caller waits**
   ...
   refuses it.
5. **`derive` is a closed list.** `aliases` was its only member in v0.61;
   v0.75 adds `scent` under its own contract (J.13.6.1).
...
Rule 1 above is a guarantee about `aliases` and it stays exactly as
written: that derivation is arithmetic on committed material and must
never reach a model. G.4.2 and G.4.3 changed a derivation that is not
arithmetic — the tags and aliases the Curator writes from the DOCUMENT —
and every forest already ingested carries the old, thinner scent. Without
a path to it, the whole of v0.75's ingest work would apply only to
documents nobody has ingested yet, and the forests that need it most are
the oldest ones.

`derive: ["scent"]` is therefore a second member of the closed list, with
its own contract, deliberately not folded into `aliases`:

1. **It reads the node's own stored body, and calls the forest's ingest
   model.** No source tree, no converter, no re-conversion — the body in
   the forest is what curation gets, exactly as it does for a document
   arriving through G.7.4. This is the one member of `derive` allowed a
   model call, and it says so rather than quietly widening rule 1.
2. **It is a J.9 job, never a synchronous wait.** `aliases` recuration is
   arithmetic and the caller waits; this is one model call per node, so on
   a 1,877-node forest the synchronous shape is a request nobody can hold
   open. It gets the job record, the stage reporting (G.10.1), the cancel
   and the one-batch-per-forest lock every other ingest-shaped run has —
   and for the same reason, since it is an ingest-shaped run.
3. **Summary is replaced; tags and aliases are UNIONED.** Re-curating
   means re-writing the scent, and the summary is the field the pass
   exists to improve. Tags and aliases are not: they may have been
   corrected by a person (J.5.18) or taught by the operator's `aliases:`
   map, and a model MUST NOT delete curation a human wrote. Removing a
   bad tag stays a human act with a human's authority behind it.
4. **G.4's fallback rule holds.** A model that fails, refuses or returns
   an invalid summary leaves that node exactly as it was, counted as a
   fallback in the report. A re-curation pass MUST NOT be able to leave a
   forest worse than it found it.
5. **The cost is stated before it is spent.** The response that starts
   the job MUST carry how many nodes are in scope, because that number
   is also the number of model calls the operator is about to pay for.
   J.10.8's rule, applied to a batch: the budget is said whatever chose
   it.
6. **`admin` on the forest and an unrestricted scope**, as J.13.6 rule 4
   already requires, and it commits, so a read-only Station refuses it.
7. **In Studio it is its own card in Optimize**, and it MUST name the two
   things that separate it from its neighbours: it spends model calls,
   and it changes what a node SAYS rather than what finds it.

**F.166 (acceptance).** The repair reaches a forest already ingested. A
forest planted with the v0.74 curator — thin tags, no model-written
aliases — gains both after `derive: ["scent"]`, and `locate` answers a
question it could not answer before. A tag added by a person and an alias
from the operator's `aliases:` map are both still present afterwards: the
pass unions and never deletes curation. A node whose model call fails is
left byte-identical and counted as a fallback. The job reports the number
of nodes in scope before any model call is spent, and a read-only Station
refuses the route.
...
listing names with a filename header and an octet-stream type. Since
v0.74 that is a container (`.forest`); a snapshot taken before it is
still served as the bundle or the sidecar it was written as.

- **One snapshot is one row and one download (v0.74).** A listing that
  offers two controls for one backup is a listing that invites the halves
  to be fetched apart, and the console is where that separation actually
  happened. A pre-v0.74 pair keeps both controls — the halves are real
  and hiding one would lose it — but nothing produced from now on has
  two.
...
snapshot itself in the request body, and answers with the forest the
registry now serves. The snapshot is a v0.74 container, or a pre-v0.74
bundle with its sidecar in a second field.
...
- **The archive carries payloads, and the consumer decides that (v0.50;
  the container, v0.74).** Part I's producer writes only payload files,
  and only under `payloads/`, but an imported archive came
  ...
  files an archive exists to carry) rather than filtered against
  known-bad shapes; the extraction's destination is a working git
  repository, whose contents git itself later reads and acts on, and
  discarding relative segments is not a sufficient rule there. An
  explicit uncompressed ceiling applies, since compression ratios are not
  bounded by the upload cap above.

  The positive naming MUST survive the v0.74 widening, and widening it is
  what makes the rule easy to lose. A container member lands under
  `payloads/` and nowhere else; its remaining path may hold no `..`, no
  leading separator, no backslash **and no component beginning with a
  dot** — `payloads/.git/config` reaches the repository git reads on the
  next commit without using a single relative segment, which is exactly
  the failure the paragraph above describes. Inside that, a member is a
  dataset database or a file under `_assets/`, because those two are what
  A.3.1 keeps out of git and therefore all a snapshot has to carry.

- **The shape is read from the file's content (v0.74).** Whether the
  upload is a container or a bare bundle is decided by its first bytes.
  The filename arrived with the request, so on this route it is a claim
  by whoever is importing, and a claim is not a format. Nothing else
  about the import changes: a container's bundle member is restored
  exactly as a bare bundle is.

- **The import states the hole it created (v0.74).** The response carries
  the count of passports naming a local payload that is not on disk,
  beside the count of payloads restored (Part I). An import that produces
  dead datasets is allowed — a pre-v0.74 bare bundle has no payloads to
  give, and refusing it would make yesterday's backups unrestorable — and
  it is the console's job to show the number, not the host's job to
  decide the operator did not mean it.
...
**F.158 (acceptance).** A container round-trips. `create` on a forest
holding a dataset payload and an `_assets/` file produces ONE file whose
members are `forest.bundle`, `README.txt` and that payload set under
`payloads/`; restoring it yields a forest whose `git log` equals the
source's and whose payload files hash-equal the originals. Extracting the
container by hand and running `git clone` on `forest.bundle` alone still
produces the tree — a container that needs this software to open fails
this, whatever else it does.

**F.159 (acceptance).** The loss is counted at both ends, and the two
zeroes are distinguishable. `create` with payloads excluded, on a forest
that has them, states how many it left out; a forest that has none states
zero; the results differ. A restore of a snapshot whose payloads are
absent reports the number of passports naming a local payload that is not
on disk, and the same restore with the payloads present reports zero. A
forest whose only payload references are remote (G.9) reports zero in
both, since those bytes were never local.

**F.160 (acceptance).** The shape is read from content. A container
renamed to `.bundle` and a bare bundle renamed to `.forest` both restore
correctly, through the CLI and through J.13.2; neither consults the
extension. A pre-v0.74 snapshot — bare bundle plus separate sidecar —
still restores through both.

**F.161 (acceptance).** Members are refused, not skipped, and the
positive naming survives the widening. Each of: a member outside
`payloads/`; a relative segment; a component beginning with a dot
(`payloads/.git/config`); an absolute path; a backslash; and a file that
is neither a dataset database nor under `_assets/` — refuses the whole
archive and leaves no forest behind. The uncompressed ceiling still
applies and names itself when it refuses.
...
**The `answer` teaching follows the token (v0.82).** The core's
`answer(forest, question)` bullet — the call, `min_evidence`, `min_score`,
`detail`, `hops`, and `view` for a `media:` reference in the reply — is
emitted only for a key that holds `answer` (J.2.7) on some selected forest;
otherwise the core names the call in one line with the capability it needs,
as the reference table already annotates a block chosen beyond the key. A
key that cannot ask must not be taught to ask.
...
fails. A sweep draws a flight and one step per hit and no hop sequence; a
walk draws the hop sequence, and a hop that returned a set contributes no
position to it.

**F.153 (acceptance).** The camera's frame is computed from the revealed
set, not the final one: a build that leans on every marked node from the
first frame fails. Navigating the view still spends nothing (rule 10).

**F.154 (acceptance).** An audit row carries what J.4.2 says it carries. A
refused call records its envelope's `error_code` and no part of the
message; a primitive the engine ran records `ms` equal to the Part D slice
the same call reported in `Server-Timing: vine`; an `answer` that reached a
provider records `model_ms` and, when the binding is priced, `usd` and
`tokens` equal to the ones the response carried. A call that reached no
provider carries no cost fields at all, and a row written before this
version reads every new field as absent — a test asserts absent rather
than zero, because the whole point of the rule is that the two are
different claims. Covered by tests.

**F.155 (acceptance).** `totals` describes the filtered set and not the
page: the same request asked with `limit=1` and with `limit=500` returns
the same `calls`, `errors`, `usd` and `people` while returning different
numbers of entries. A store hit's cost appears in the avoided figure and
in no spend. Every filter of J.4.3 narrows both the entries and the
totals — a filter honoured on one and not the other fails this — and an
unreadable `since` or `until` is `E_SCHEMA` rather than a filter quietly
not applied. Covered by tests.

**F.156 (acceptance).** The scope of J.3.2 survives aggregation. An
administrator of one forest reads totals counting only their own rows: a
call made on a second forest changes neither their `calls` nor their
`usd`, and the facets J.4.3 requires name only the forests, people and
primitives that appear in their own set. Governance rows are counted for
the owner alone. Covered by tests.

**F.157 (acceptance).** The Audit console reads the host's totals rather
than summing its page, keeps a store hit's cost out of the spend, renders
an unknown error code as itself, and carries every filter in the address.
Read off the source on F.137's boundary — the numbers are the route's and
the route is tested; what is checked here is that the console did not grow
a second opinion about them. Covered by tests.
...
### J.18 The uncertain links, read and voted on (v0.75)

H.2.1 gives a person the vote; this is where they cast it. Two routes and
one console.

| Endpoint | Returns |
|---|---|
| `GET /v1/forests/{f}/links/uncertain` | the managed links, with both endpoints' scent |
| `POST /v1/forests/{f}/links/vote` | `{votes: [...]}` accepts or rejects them |

Normative rules:

- **Both ride the `write` capability, on the forest, at the caller's own
  scope.** Not `admin`: this is a per-node frontmatter edit, and a
  principal who may write inside a branch may settle the proposals inside
  it. A link with an endpoint outside the caller's scope is absent from
  the listing and refused by the vote, in both cases byte-identically to a
  link that does not exist (H.2.1 rule 6).
- **The listing carries what a decision needs and nothing else.** Per
  link: the two node ids with their `type`, `title` and `summary`, the
  `rel`, the link's `confidence`, the proposal's `note` when it has one
  (G.4.2.1 rule 3), and **the persistent heat of both endpoints**. The
  heat is there because it is the answer to "why has this not been
  promoted already" — without it the operator is asked to second-guess a
  rule they cannot see. No bodies: a decision about adjacency is made on
  the scent, which is what the proposal itself was made on.
- **Grouped by source node, and paged with a cursor.** Proposals are born
  up to three at a time on one document (G.4.2.1 rule 4), so that is the
  unit a reviewer actually reads. `after` + `limit` in scan's shape; the
  order is stable so a page is not reshuffled by another principal's vote
  mid-review.
- **A vote batch is at most 50, and it is NOT all-or-nothing.** C.7.4's
  plant batch is atomic because a branch and its children are ONE thought
  that is incoherent half-written; fifty votes are fifty independent
  decisions, and failing all of them because one target had since been
  pruned would throw away work a person actually did. Each vote is its own
  `.md` commit (H.2.1 rule 1), and the response reports the outcome of
  every vote sent: `accepted`, `rejected`, `unchanged` (already settled —
  idempotent, never an error), `missing` (the link, or an endpoint, is
  gone or out of scope), or `refused` with the code.
- **Audited per vote** (J.4), stamped with the acting principal like every
  other hosted write, and emitting `node.grafted` (J.16) — the payload
  rules are unchanged, so it carries the two ids and the rel and never a
  summary or a note.
- **A read-only Station refuses the POST and serves the GET.** Reviewing
  what is pending is reading; settling it is a commit.
- **The console lives beside the report that counts them.** J.5's Health
  console already shows `uncertain_links` as a bucket count and no more,
  which is the screen that makes an operator ask the question this route
  answers; the count becomes the way in. The console MUST show both
  endpoints' summaries side by side, MUST let a reviewer settle a whole
  source node's proposals in one action, and MUST NOT offer "accept all"
  over a page the reviewer has not read — a control that settles what is
  off screen turns a review into a rubber stamp, and the confidence it
  writes would then record nothing.
- **The queue's emptiness is a fact worth stating** (J.5.4's rule): a
  forest with no managed links says so, rather than rendering as a
  console that failed to load.

**F.165 (acceptance).** The vote settles what heat cannot. On two nodes
with zero heat carrying a `related-to` proposal at 0.3 — under a
configuration whose `prune_below` sits beneath it, so that a full Ranger
cycle run first provably leaves it unchanged; the shipped default is
pinned by its own companion test as the OPPOSITE fact, a cold cycle
deletes the proposal — an `accept` raises the link to 1.0 in one
`.md` commit stamped with the principal, and a subsequent Ranger run does
not touch it. A `reject` removes it. Voting again on a settled link
answers `unchanged`, not an error. A batch of 50 where one target has
been pruned settles the other 49 and reports that one as `missing`. A link
whose far endpoint is outside the caller's scope is absent from the
listing and refused by the vote byte-identically to a link that does not
exist. A vote on a `confidence: 1.0` link or a structural edge is refused.
And the MCP tool list contains no name that can cast a vote — the surface
is the host's, and F.165 asserts its absence.
...
## Part L Extensions (third-party modules, v0.80)

### L.0 Why an extension is not a fork

Part G opens its extensibility section with a sentence that has held since
v0.9: "UIs and bots are MCP/library clients, not plugins." That sentence
drew the line in the right place and left a class of work with nowhere to
stand — the work that is neither a client nor a change to the contract, but
a **capability the deployment does not have**: transcribing an audio file,
converting a format nobody here has heard of, reading a proprietary export,
describing an image with a model this project never integrated.

Every one of those arrives with a dependency, and the dependency is the
whole problem. FFmpeg is GPL or LGPL depending on how it was built; a
document converter drags a machine-learning stack; a transcriber drags
another. `LICENSING.md` splits this tree Apache-2.0 (engine) / AGPL-3.0-only
(host), and that split holds only while the dependency list stays clean.
Pinning any of them makes this project their distributor.

An extension moves the decision to the operator, and the direction of that
move is the entire point. **This project MUST NOT distribute an extension's
bytes** — the curated index of L.2 carries signed *pointers*, never
artifacts. What is installed on an operator's machine is the operator's act,
under the extension's own licence, which MUST be shown before the install
completes.

The shape is a browser extension written in Python: one package carrying its
backend module *and* its console surface, declaring what it contributes,
installed and removed by the operator. It contributes at named seams; it
does not patch. That distinction is what leaves this project free to
refactor: an extension is coupled to a seam's published contract, never to
the shape of the code behind it.

### L.1 The package

An extension is a directory (delivered as a zip, or as a git tree — L.2)
whose root MUST carry `manifest.json` and MUST carry the module its manifest
names. Everything else is optional.

```
manifest.json         # identity, compat, permissions, contributions, config schema
main.py               # register(api) — the one activation entry point
worker.py             # functions declared heavy (L.5)
ui/panel.json         # declarative console contributions (L.11)
requirements.txt      # resolved into the extension's own environment
LICENSE               # the extension's own licence
```

The manifest MUST carry `id`, `version`, `station_compat`, `license` and
`contributes`. `id` MUST be unique within a deployment and is the namespace
for everything the extension creates (L.4 rule 5).
...
```json
{
  ...
  "models": {
    "requires": [],
    "registers": [{ "role": "transcribe", "kind": "transcribe",
                    "description": "Turns a recording into text." }]
  },
  "contributes": {
    "converters": [
      { "extensions": [".mp3", ".m4a", ".wav"],
        "handler": "worker:transcribe", "heavy": true }
    ],
...
**`station_compat` is a range against the host's published version** — the
same string `forests()` already serves (J.1.2 rule 6). An install whose
range excludes the running host MUST be refused by name and version, never
...
| uploaded archive | the operator's own bytes, from a browser (v0.81) |

**The uploaded archive is the door the others do not open.** "A local file"
is a path on the **host**, which through a browser is the container's
filesystem — so an operator holding an extension they just wrote had no
route to a remote Station short of publishing it to git, and the local-file
door served only somebody with a shell there, who would have used the CLI.
Bytes arrive as `{name, b64}`, the shape J.8 has carried since v0.48, and
are written into the workdir as the archive L.2 already resolves: a fifth
door, not a fifth resolver.

Two rules ride with it. It is **`unverified` by construction** — an upload
carries no forge and no index identity, so there is nothing that could be
verified and the tier MUST say so rather than implying a check happened.
And it is **bounded** (`MONKEYLLM_STATION_EXT_UPLOAD_MAX_MB`, 25 by
default), because this door is for an extension's own code: dependencies
are resolved at install (L.5), and a vendored-wheel bundle belongs to the
local-file route, where the bytes are already on the host.

**Rule 1 — the source resolves to an immutable artifact before anything is
validated.** A git ref MUST be resolved to a commit SHA, and the **SHA** is
what the install record holds. A tag can be force-pushed and a branch moves;
"the thing that was signed" MUST be the thing that runs. `@v1.2.0` means *the
SHA that tag pointed at when this operator installed*.

**Rule 2 — a moving ref is allowed and marked.** `@main` MAY be installed:
an author developing an extension must not have to cut a release per test.
The record MUST carry `tracking: <ref>` beside the SHA, and every surface
naming the extension MUST name the ref beside it — "the same extension" on
two deployments is otherwise not the same code. Being *behind* costs a
network round trip, so it MUST be reported on demand (`vine ext outdated`)
and MUST NOT be computed by a listing, which MUST stay an offline call.

**Rule 3 — three trust tiers, recorded and shown.**

| Tier | Meaning |
|---|---|
| `verified` | id from the curated index; the index carries the expected signing identity and the artifact matches it |
| `signed` | git source at a **signed tag**, verified against the public keys the forge publishes for the author's account |
| `unverified` | everything else — unsigned tags, tracked branches, plain archives, local files |

Two consequences follow rather than being decided: a **tracked branch is
`unverified` by construction** (a branch carries no tag, so there is nothing
to verify), and **`signed` requires the network at install time**, because
the keys live at the forge — an air-gapped deployment installing from a
local file reaches `unverified` or `verified`, never `signed`.

The tier MUST appear everywhere the extension appears. An extension MUST NOT
change tier quietly: a source that was `signed` and comes back unsigned MUST
refuse the update until the operator acknowledges it.

**Rule 4 — the verifying identity is recorded.** Delegating identity to the
forge is what avoids running a key infrastructure, and its cost is stated
rather than hidden: **a compromised forge account is a compromised
extension.** The install record MUST hold the account and the key
fingerprint that verified it, and a change on update MUST be surfaced, not
accepted silently. Repo transfers and key rotations become visible without
anybody maintaining a keyring.

**Rule 5 — what validation cannot promise.** Resolving `requirements.txt`
executes third-party build code by construction — that is true of pip
everywhere — and a git source additionally runs the author's build backend.
The conformance kit (L.9 rule 1) therefore does **not** run before
third-party code runs; it runs before anything is **registered**. The
documentation MUST say this in those words, because it is the reason the
signature tier carries more weight than the kit does.

### L.3 The seam catalogue and precedence

An extension **claims** a seam; it MUST NOT patch, wrap or replace anything
this specification did not name. A seam absent from this catalogue is not an
extension point, and reaching one is a defect regardless of whether it
works.

| Seam | Contributes | Precedence |
|---|---|---|
| `converters` | a file extension → a document (G.2) | operator command hooks > extensions by install order > entry points > built-ins |
| `curation` | an `on_curate` hook (G.4) | all hooks run, in install order |
| `events` | a handler for `plant`/`graft`/`prune`/`ingest`/`answer` | all handlers run, order unspecified |
| `jobs` | maintenance beside the Ranger (H), **pulled** not scheduled | independent |
| `tools` | an MCP tool (C, J.1) | namespaced; collision refuses the install |
| `routes` | a REST route under `/v1/ext/<id>/` (J) | namespaced by the path, so a contribution can never take a name the product needs |
| `panel` | a console contribution (L.11) | one per extension |
| `ranking` | an ordering contribution to `locate`/`harvest` | first claimant wins |
| `prompt` | a contribution to the `answer` prompt (J.10) | first claimant wins |
| `roles` | a model role and its `kind` (L.6) | collision refuses the install |

`discover_converters` (G.2) already works this way — config command hooks >
injected extras > entry points > built-ins — so the model is not new. It is
the existing one, published with a version.

**Every seam DECLARES its handler's contract (v0.81).** What a `curation`
handler receives, what an `events` handler receives and what each must hand
back were prose here and nowhere in the code, so the conformance kit could
answer "it works" about an extension whose handler took the wrong
parameters — and the author learned otherwise on the first ingest, at
whatever hour that ran. The declaration lives beside the seam catalogue,
the kit checks a handler's signature against it **at install** (L.9 rule
1), and the authoring reference (L.16) is generated from the same
declaration rather than transcribed beside it.

The rule is about **defaults**, not about `**kwargs`. A handler MAY omit
any parameter it does not use; a parameter the seam does not pass is fine
when it carries a **default** (it simply keeps it) and is a fault when it
does not, whatever else the signature accepts.

`**kwargs` excuses nothing, and the first draft of this paragraph said it
did — wrongly, in the exact case it was written for. Given `def
on_event(event, forrest, **rest)` the host passes `forest`, `**rest`
absorbs it, and `forrest` is still unfilled: a `TypeError` at call time,
which for a `curation` handler is whatever hour the next ingest runs. That
is the silence this check exists to break, so the check may not be excused
by the construct that hides it.

The check MUST read the author's **source** rather than import it: a
`heavy` handler's module imports the very dependency L.5 keeps out of this
process, so a check that imported would break the rule that protects the
deployment in order to catch a typo — and it would run third-party code at
kit time, which L.2 rule 5 is careful to say the kit does not do.

**The last two seams carry a rule the others do not.** A `ranking` or
`prompt` contribution changes what the product answers, and an operator
comparing a bad answer against a good one has no way to know an extension is
between them. Both MUST therefore be **named in the Part D trace** of every
call they touch, and the trace's consumers (J.10.4 `explain`, J.5.15, the
Ask console) MUST render that name. A contribution that cannot be attributed
MUST NOT be admitted.

Each of them is also bounded by what it may not do:

- A **`ranking`** contribution may reorder and drop; it MUST NOT invent. A
  result it returns that was not handed to it MUST be discarded — G.4.2.1's
  rule about hallucinated targets, applied to ordering. It runs **before**
  the budget, so what it promoted is what survives the cut rather than being
  reordered inside a set somebody else already chose.
- A **`prompt`** contribution is **appended**, never substituted. The rules
  the product's own answers depend on — citation, the refusal to invent, the
  stated budget — MUST NOT be removable by an extension.
- Either one raising leaves the call exactly as it would have been without
  it (L.7 rule 5), which for `ranking` is the original order and for
  `prompt` is the unmodified system message.

### L.4 `register(api)`

An extension is activated by exactly one call, `register(api)`, in the module
its manifest names. It MUST NOT act at import time.

`api` is **enumerated**: it exposes what this specification lists and
nothing more, and a new engine capability requires its own line here before
an extension can reach it. That is the same construction `ScopedVine` uses,
for the same reason — a surface that forwards whatever it is given is not a
surface, it is an accident.

1. **`api.contribute(seam, handler, **spec)`** — the only way to claim a
   seam. Claims made outside `register` MUST be refused.
2. **`api.config`** — the extension's own settings, validated against the
   manifest's `config` block. A field declared `secret: true` is readable
   here and MUST NOT be readable anywhere the extension can export it (L.7
   rule 4).
3. **`api.models.complete(role=..., ...)`** and
   **`api.models.transcribe(role=..., audio=...)`** — L.6. The extension MUST
   NOT receive an endpoint, a key, or `has_key`. Two methods rather than one
   because they are two APIs: a chat turn is a JSON body to
   `/chat/completions` and a transcription is a multipart upload to a
   different path. Collapsing them would put the difference in a caller's
   arguments, where the binding cannot see it — which is the whole reason a
   role carries a `kind`.
4. **`api.vine`** — a `ScopedVine` carrying **the caller's** scope, never a
   raw `Vine` and never a widened one. Every J.3 guarantee survives an
   extension unchanged: an extension tool asked for an out-of-scope id MUST
   return the byte-identical `E_NOT_FOUND` a primitive returns.
5. **`api.namespace`** — every id, tool name, route and table the extension
   creates MUST begin with it. A tool named outside the namespace MUST
   refuse the install rather than be renamed silently.
6. **`api.log`** — the host's own log, tagged with the extension id.

An extension's module MUST be executed **once per load**. A loader touches
it at least twice — for `register` and again for each declared handler — and
executing it twice gives the extension two module objects: `register()`
writes to one and its handlers read from the other, so any module-level
state the author keeps (a client, a cache, a counter) silently splits in
half. Nothing raises, which is exactly why the rule is written down.

Where a process legitimately holds two installs of one `id` — a test that
builds several hosts, a staging copy beside a live one — they MUST be
distinct modules. The identity of a loaded module is therefore the installed
**tree**, never the `id` alone.

### L.5 The worker

The manifest MAY mark a handler `heavy: true`. A heavy handler MUST run in
the extension's own environment, in a separate process, and MUST NOT be
imported into the host's interpreter.

This is what keeps the dependency that motivated the extension out of the
process that serves the forest: a transcriber's machine-learning stack, a
converter's document toolchain, a binary that is not Python at all. The host
MUST enforce a wall-clock timeout and MUST treat a worker's death as a
handler failure (L.7 rule 5), never as a host failure.

The worker protocol MUST be language-neutral in shape — a request and a
response over a pipe, values that survive JSON — so that a future
non-Python worker is a packaging question and not a contract change. This
version ships one SDK.

### L.6 Models: roles, never keys (amends J.10)

An extension MAY **require** roles the deployment already binds, and MAY
**register** a role of its own.

- **A registered role appears wherever bound roles appear** and is bound by
  the operator to a provider they already pay for. The extension therefore
  needs no key of its own, and the deployment keeps one place where model
  spend is configured.

  **Rule 1 (v0.83): appearing means bindable, on every surface.** The
  bind route MUST accept every role a loaded extension registers, and the
  read beside it (`GET /v1/admin/models`) MUST carry `roles`: every
  bindable role with its `kind`, whether it is the product's own
  (`builtin`) or an extension's (`ext`), and its `description` — the
  manifest's for a registered role (L.1). A console MUST render its role
  cards from that list and MUST NOT keep one of its own, and MUST shape
  the binding form by `kind`: a `transcribe` binding has no reply length
  and no reasoning switch, because the call is a multipart upload and not
  a chat turn. The v0.82 Station registered the role, listed it on the
  Extensions console, and refused to bind it on the Models console and on
  the route alike — the sentence above was true of the list and false of
  the act.
- **A role carries a `kind`** — `chat`, `embed`, `vision`, `transcribe`,
  `rerank` — because transcription is not `/chat/completions`. Without
  `kind` an entire class of extension has nowhere to bind, and the binding
  UI cannot know what to ask for.
- **A required role with no binding MUST refuse the install**, by name, at
  install time. Discovering it at the first transcription is discovering it
  in front of a user.
- **The extension MUST NOT receive credentials.** `api.models` hands it a
  bound caller; the endpoint and the key stay under J.10.2's custody
  exactly as they do for every other consumer.
- **Every extension model call MUST be metered** and MUST reach the J.4.2
  row with its `usd`/`tokens`/`model_ms`. Unmetered extension spend is a
  bill with nobody's name on it, which is the failure J.4.2 was written
  against.

### L.7 Authority, audit and quota (amends J.4.2)

1. **The owner installs; a forest's admin enables.** Installation puts
   third-party code in the host's process and therefore within reach of
   every forest, so it MUST require the authority that covers every forest
   (J.10.2's reach rule). Enablement decides what acts on one forest's
   material and MUST require that forest's `admin`. An admin MUST NOT be
   able to install: a local decision with a global effect is not a local
   decision.
2. **Enabled on a forest means published on that forest.** An extension's
   tools and routes MUST be visible exactly where it is enabled; a key
   reaching several forests sees the union. There is no second opt-in
   control, and there MUST NOT be one — J.1.2 rule 4 already says a surface
   with nothing behind it is not announced.

   MCP's `tools/list` is per **connection** while enablement is per
   **forest**, so the menu a key is served MUST be the union of the
   extensions enabled on the forests **that key reaches**. Two consequences
   are intended rather than tolerated: two keys on one deployment can be
   served different menus, and an extension enabled nowhere the key reaches
   is **invisible** rather than merely refusing — a tool that can only ever
   refuse costs every session its description and teaches nothing.

   The per-call check is a **separate** decision and is the real gate:
   listing answers *what is worth showing*, calling answers *what is
   allowed*. A tool called on a forest that has not enabled its extension
   MUST answer the byte-identical `E_NOT_FOUND` an unknown tool answers —
   J.3's oracle does not acquire an exception because the caller guessed a
   name. And because the filter decides what a key is shown, an
   implementation that cannot apply it MUST withhold every extension tool
   and say so: fewer tools than the rule allows is a degradation, more is a
   disclosure.
3. **The audit row keeps the human and names the extension.** J.4.2's row
   gains **`via`** (`ext:<id>`), nullable, emitted only when an extension
   was in the path. `principal` MUST remain whoever caused the act — a
   forensic question is *who asked for this*, and an extension is not an
   answer to it. A row written before this version reads `via` as
   **absent**, never as "no extension", which is J.4.2's own rule about
   fields that lie by omission.
4. **Config is stored under the host's custody, and a secret never leaves
   it.** A field declared `secret: true` MUST NOT be returned by any read
   surface (`has_value` is the answer), MUST NOT be written to a versioned
   file, and MUST NOT be included in a webhook payload, an export, a
   snapshot or a log line. On a host with no registry (L.12) a declared
   secret MUST come from the environment and the loader MUST refuse a
   versioned config file that carries one.
5. **A raising extension is contained.** A failure inside a seam MUST fall
   back to the behaviour that would have applied without the extension —
   the built-in converter, the unmodified ordering, the absent hook — and
   MUST be reported (the ingest report, the trace, the log). This is G.5.1's
   rule for a describer that raises, generalised: an extension may fail to
   help, and may not fail the act.
6. **Quota is per extension per forest, and exhaustion is a refusal.** The
   operator MAY set a ceiling on an extension's model spend; reaching it
   MUST refuse in the envelope shape (`E_EXT_QUOTA`) and MUST be audited.
   Silence at the ceiling would be a stop nobody could distinguish from a
   bug.

### L.8 Lifecycle

Install → restart → enable per forest → run → disable → uninstall →
restart.

**The restart is required and MUST be stated.** Python does not unload a
module, so "disabled" without a restart means the code is still resident.
The host MUST NOT imply an unload that did not happen; a console MUST say
"disabled; memory released at the next restart". On a CLI host the
requirement costs nothing — every invocation is already a fresh process.

**A first install MAY be active at once (v0.83).** The restart is about
*unloading*. An `id` this process has never loaded has nothing to unload,
so a host MAY activate it in the running registry the moment the install
record is written, through the same loader and the same per-forest
enablement every other extension goes through. The install response MUST
say which happened — `activated: true` with `restart_required: false`, or
the reverse — and the listing MUST carry `loaded` per extension, so an
operator can tell "installed" from "installed and serving". Reinstalling,
updating or removing an id that IS loaded keeps the restart, because that
is the case Python cannot honour. An activation that fails MUST NOT fail
the install: the record stands, `restart_required` is `true`, and the
failure is named.

**Uninstall touches nothing in a forest.** Nodes stay, the git history stays,
the passports stay. But a `type` or `rel` the extension declared in a
forest's dialect (A.2) stops being declared, and A.2 then refuses the next
write that uses it. The host MUST therefore **name what will start being
refused** — every token the extension declared that no other extension
declares — before the operator confirms.

**Config is quarantined, not deleted.** A failed upgrade must be
recoverable, so config survives uninstall marked orphaned and is recovered
by reinstalling the same `id`. Quarantined secrets MUST expire and MUST be
listed somewhere an operator can see them: a secret that outlives its
extension and is visible to nobody is the worst of both choices.

### L.9 The four controls

Each does a different job, and conflating them is how a deployment comes to
believe it is protected by the one that does not protect it.

1. **Conformance kit** — the manifest against the schema, `station_compat`
   against the host, declared handlers importable **and matching their
   seam's declared contract (L.3, v0.81)**, a document round-trip against
   the fixture forest, and L.10's configurability rule. This is a
   **compatibility** control. The documentation MUST say plainly that it is
   not a security control: it establishes that an extension works, never
   that it is honest.
2. **Declared permissions** — network hosts, filesystem, capabilities,
   granted item by item at install. In the host's own process these
   **inform**; only what runs in a worker (L.5) can be **contained**. An
   install screen MUST NOT imply otherwise.
3. **Signature and the curated index** — L.2 rules 3 and 4.
4. **Operator sovereignty** — the licence, the permissions, the source and
   the tier are shown, and the operator decides. Nothing installs
   unattended.

### L.10 A contribution the host cannot serve

A CLI host has no console, so a declared panel has nowhere to draw. The rule
is J.1.2 rule 4 applied to extensions: **a contribution the host cannot
serve is inert and named, never an error.**

- The loader MUST parse and validate every contribution, including those it
  cannot serve, so a broken panel is caught on the author's machine instead
  of in a Station.
- It MUST NOT render it; it exposes the parsed contribution and a host that
  has that surface draws it.
- A listing MUST say so — `panel: declared (no host surface)`.

The half that matters more is configuration. **The config schema is the
source of truth and a panel is decoration over it.** A console renders a
form from the manifest's `config` block; a CLI reads the same block to set
and validate the same values. A registered role binds through the models
console on one host and through the environment on the other — one role, two
binding surfaces.

The conformance kit MUST enforce it: **an extension with a required setting
that has no entry in the config schema fails.** That single rule is what
keeps a console-less host first-class rather than degraded.

A panel-only extension is legitimate and simply does nothing where there is
no console. The listing says so rather than implying it is running.

### L.11 The console surface (amends J.5)

An extension's console contribution is **declarative**: the manifest
describes panels, fields, tables and actions, and the console renders them
with its own components.

This is not a stylistic preference. J.5.13 admits script only by hash
computed from the built shell, with no `unsafe-eval` and `connect-src
'self'`; a third-party script inside the console's origin would run with the
operator's session against every forest they can reach, which is the
vulnerability class this rule exists to prevent. **The console MUST NOT
execute extension-supplied script in its own origin**, and MUST NOT relax
its CSP to admit one.

A vocabulary this specification has not named cannot be drawn. That is the
accepted cost, and it is bounded: the vocabulary grows by amendment, and an
extension needing more than it offers still has its REST route and the
operator's browser.

**The install is one legible act (v0.83).** The console that installs an
extension is used by the person least able to read a mechanism, so the
shape of the act is normative:

1. **The primary action sits beside the thing it acts on**, and it is the
   only primary control on the card. A file chosen at the bottom of a card
   and a button at its top right are two places for one act, and the
   v0.82 console shipped exactly that: an operator chose a zip, saw the
   distant button change shade, and did not read it as the next step.
2. **Choosing is not an act; reviewing is.** Typing a source or choosing a
   file changes nothing on the host. The review (L.9 rule 4) is the first
   call, and its result — licence, source, tier, permissions, what it
   contributes, what it registers — MUST be shown **in place**, under the
   choice that produced it, with the install control beside it. A dialog
   that opens over the card is a third place.
3. **Enabling MAY ride the install.** Installing from a forest's console
   MAY enable the extension on that forest in the same act, when the
   principal administers it, through a visible control that defaults ON —
   installing from a forest is a statement of intent about that forest.
   The route takes `enable_on: <forest>`, applies it after the record is
   written, by the enablement path L.7 rule 2 already governs, and never
   for a forest the principal does not administer.
4. **The outcome says what to do next.** After the install the console
   MUST state, in this order: whether the extension is active now or
   after a restart (L.8); what it now accepts (formats, from J.8.5) and
   what it now offers (roles, tools); and what is still missing before it
   can be used — a role to bind, named, with the way to the console that
   binds it. An install that ends in a green tick and no sentence is the
   install the operator reported.

### L.12 Where the mechanism lives

Part J calls the Station a *privileged client* of the engine — "the engine
gains nothing, loses nothing". Extensions are not the exception. **The
mechanism is the engine's; the governance is the Station's**, which is the
cut that already separates `Vine` from `ScopedVine`.

| Engine | Station |
|---|---|
| manifest schema, source resolver, tiers, signature | owner/admin authority (L.7 rule 1) |
| conformance kit | secret custody (L.7 rule 4) |
| environment resolution, loader, `register(api)` | per-forest quota (L.7 rule 6) |
| seam precedence, worker protocol | `via` in the audit row (L.7 rule 3) |
| the CLI: install, update, list, config, enable, remove | the console (L.11), the curated index |

It follows that **an operator with no Station installs extensions**, and
that is required, not incidental: the engine is the artifact that spreads,
and a capability reachable only through the AGPL host would make every
extension author a client of that host.

Two rules make it work:

- **Global install is a host-level directory; per-forest enablement is a
  line in the forest's `_meta/`.** Enablement is therefore versioned and
  travels in a snapshot (Part I), so a forest carries **which extensions it
  expects**. `validate` MUST report an expected-but-absent extension —
  otherwise an `.mp3` is converted by the built-in stub and nobody is told.
  Installation state MUST NOT live in `_derived/`, which is disposable and
  which `reindex` may delete.

  Writing that line MUST also **commit** it. A snapshot is a git bundle, so
  a file merely sitting in a versioned directory travels in nothing, and
  "it is versioned" would be true of the path and false of the artifact.
  A.3.1's guard on the ordinary commit path — only `.md` is staged, so
  binaries cannot enter forest git — is NOT relaxed for this: the write
  goes through a second, narrower door that admits `.yaml` and `.md`
  **under `_meta/` only**. A guard widened for a convenience is a guard
  that has stopped meaning anything, and the capability this needs is small
  enough to be named rather than implied.
- **`_meta` declares expectation and never grants it.** A forest that
  expects an extension the deployment has not installed MUST appear in the
  health report; it MUST NOT cause an install.

The loader MUST be optional (`monkeyllm[extensions]`). Absent, the base
install MUST load with the loader inert — never an import error — because
an embedder of the engine did not ask for environment management.

### L.13 Boundaries that stay closed

Unchanged, and an extension reaching any of them is a defect:
...
- **J.3's oracle.** An extension tool MUST NOT disclose the existence of a
  node outside its caller's scope, by any channel including timing-free
  ones like a count or a distinct error.
- Binaries never entering forest git (A.3.1).
- A new node `type` or `rel` is declared in the forest's dialect (A.2),
  through the seam that already exists, never around it.

### L.14 Acceptance criteria

**F.174.** A manifest failing the schema, `station_compat`, or the
conformance kit installs **nothing**: no environment, no record, no
registration.

**F.175.** A heavy handler's dependency is absent from the host's own
interpreter — asserted by importing it in-process and expecting failure —
while the handler itself runs.

**F.176.** An extension requiring a role with no binding refuses the install
and names the role.

**F.177.** An extension model call reaches the J.4.2 row with the human
`principal`, `via: ext:<id>`, and its cost and `model_ms`.

**F.178.** Quota exhaustion refuses in the envelope shape, is audited, and
the following period admits again.

**F.179.** An extension tool asked for an out-of-scope id returns the
byte-identical `E_NOT_FOUND` a primitive returns.

**F.180.** Enabling on forest A publishes the tool to a key holding A and
not to a key holding only B; a key holding both is served the union, an
anonymous caller is served none, and calling the tool on B answers the
byte-identical `E_NOT_FOUND` an unknown tool answers.

**F.181.** Uninstall names every `type`/`rel` that will start being refused
before confirmation, and the forest's files are byte-identical afterwards.

**F.182.** Quarantined config is recovered by reinstalling the same `id`,
and expires on schedule.

**F.183.** An extension raising inside a seam is contained: ingest falls
back to the built-in converter, the batch completes, and the report names
the extension.

**F.184.** A `ranking` or `prompt` contribution is named in the Part D trace
and rendered by the trace's consumers.

**F.185.** An unsigned source installs only after an explicit
acknowledgement; the tier is recorded and shown, and a `signed` extension
returning unsigned refuses the update.

**F.186.** An extension installs and runs on an engine-only host with no
Station: install, enablement written to the forest's `_meta/`, and the
converter claiming its extension on the next ingest. The enablement is
**committed**, so a snapshot taken afterwards carries it; the ordinary
commit path still refuses a non-`.md` file.

**F.187.** `validate` names an extension a forest expects and that is not
installed; the base install without the optional extra loads with the loader
inert rather than raising.

**F.188.** A versioned config file carrying a field the manifest declared
`secret` is refused.

**F.189.** A git install records the resolved commit SHA rather than the
ref; the same `@tag` installed after the tag moved resolves to a different
SHA and the host says so.

**F.190.** An extension declaring a panel installs and runs on a
console-less host: the panel is validated, reported unserved, and nothing
errors.

**F.191.** Every required setting is reachable without a console, and the
kit fails an extension whose required setting has no schema entry.

**F.192.** A failed update leaves the previous install running and
unmodified.

**F.193.** An install from a branch records the SHA, is marked `tracking`,
lands in the `unverified` tier, and the listing names the ref while making
no network call.

**F.194.** A signed tag verifies against the forge's published keys and
reaches `signed`; the verifying identity is recorded, and an install whose
identity changed refuses the update until acknowledged.

**F.195.** An `events` contribution is called for an event on a forest that
enabled it and not for the same event on one that did not; a handler that
raises does not fail the act that emitted it.

**F.196.** A `routes` contribution answers under `/v1/ext/<id>/<name>`, is
audited with `via`, and answers the byte-identical `E_NOT_FOUND` for an
unknown name, an unreached forest, and a forest that has not enabled it.

**F.197.** A `jobs` contribution is listed and runnable on a forest that
enabled it and is absent on one that did not.

**F.198.** A `ranking` contribution reorders the sweep's results, cannot add
one that was not handed to it, and names itself on the Part D event; a
`prompt` contribution is appended to the system message and cannot remove
what was there.

**F.213 (v0.83).** `supported_formats` answers every extension the chain
claims in discovery order, naming the first claimant, and an extension
enabled on a forest appears in that forest's answer above the built-in for
the same extension while a command hook appears above both.

**F.214 (v0.83).** `GET /v1/forests/{f}/ingest` carries `formats`, and a
forest that enables a converter extension lists its extensions there while
a forest that does not enable it does not.

**F.215 (v0.83).** `GET /v1/admin/models` carries `roles` naming every
built-in role and every role a loaded extension registers, with `kind`,
origin and description; `POST` binds a registered role and refuses an
unknown name by listing what it would accept.

**F.216 (v0.83).** Installing an id not loaded in this process activates
it: the response says `activated: true`, the listing says `loaded: true`,
its roles are bindable and its formats are accepted without a restart;
reinstalling a loaded id answers `restart_required: true`.

**F.217 (v0.83).** `install` with `enable_on` enables the extension on that
forest in the same act for a principal who administers it, and refuses the
enablement — not the install — for one who does not.

**F.218 (v0.83).** The consoles carry no list of their own: the ingest
console's accepted formats, the Models console's role cards and the
Extensions console's install flow are read off the host's answers, asserted
statically on the source (F.137's boundary) with the previous console as
the negative control.

### L.16 The authoring surface (v0.81)

Part L shipped a mechanism and no way in. The only document that named
extensions described the world before them, nothing taught how to write
one, and the console offered an author no door. A contract nobody can find
the entrance to is a contract with one implementer.

**Rule 1 — prose is written, schemas are derived.** The text that teaches —
what a seam is, why an extension contributes instead of patching, a worked
example — is written by people and lives in the repository, where it is
read without running anything. The **schemas** MUST be generated from the
definitions themselves: the manifest's own JSON schema, the seam contracts
of L.3, the role `kind`s of L.6, the config field types. A transcribed
schema is a schema that lies the first time a seam is added, and it lies
silently, to exactly the person who has no other source.

**Rule 2 — the material states which host it came from.** An authoring
reference carries the host's version (J.1.2 rule 6's rule, applied to the
thing an author copies a `station_compat` range out of), so an example that
has aged says so instead of teaching a range that no longer fits.

**Rule 3 — writing is not installing.** The authoring surface MUST be
offered to anyone signed in, and MUST NOT be gated on the authority to
install. J.5.12 settled the same question for skills — "the person who may
read the forest is the person whose AI may learn to" — and it holds harder
here: an extension is written on a laptop and installed by whoever governs
the deployment, so gating the documentation on L.7 rule 1's authority
withholds it from the only person who needs it.

**Rule 4 — what it may not become.** The authoring surface is documentation
and a schema. It MUST NOT accept an extension, install one, execute one, or
carry a credential; and it MUST NOT be a second description of the manifest
that could disagree with the loader's.

**F.199.** An extension is installed from an **uploaded** archive with no
path on the host: the bytes arrive as `{name, b64}`, the tier is
`unverified`, and it installs only on an explicit acknowledgement.

**F.200.** An upload over the ceiling is refused **before** it is written,
naming the ceiling and the size; the refusal stages nothing.

**F.201.** An uploaded archive whose member escapes its root is refused as
a whole archive, never member-skipped.

**F.202.** A handler whose signature does not match its seam's declared
contract **fails the conformance kit**, naming the seam, the parameter and
what the seam passes — including when the handler also takes `**kwargs`,
which excuses nothing. A handler omitting a parameter it does not use
passes, and the check never imports the module it reads.

**F.203.** The authoring schema is **derived**: adding a seam to the
catalogue changes it with no second edit, and the served document names the
host's version.

**F.204.** The authoring surface is reachable by a signed-in principal who
may not install, and it offers no way to install, execute or upload.

**F.205.** Every seam in the catalogue has a declared contract, and every
declared contract belongs to a seam in the catalogue.

### L.15 Out of scope

A hosted marketplace with accounts or payments (the index is a signed file);
extensions in other languages (the worker protocol must not forbid one, but
this version ships one SDK); hot install or upgrade without a restart; any
relaxation of L.13.
