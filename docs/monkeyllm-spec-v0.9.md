# MonkeyLLM — Technical Specification v0.9 (Phase 0/1/2)
...
Fora do escopo da Fase 0 (não implementar): embeddings/vetores, evaporação e promoção de atalhos (Ranger), compaction de `same-as`, sync S3/R2, multi-escritor, Tropa (Parte E — Fase 1.5; apenas garantir namespace de sessão no trails.db). O ingest automático saiu desta lista na v0.9 (Parte G).
