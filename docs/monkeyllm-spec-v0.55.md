# MonkeyLLM Technical Specification v0.55 (Phase 0/1/2 + host layer)
...
Transport: MCP (stdio for dev; HTTP/SSE on Docker). All responses in JSON. Errors follow `{error: {code, message, hint}}` with codes `E_NOT_FOUND`, `E_SCHEMA`, `E_FRONTMATTER`, `E_READONLY`, `E_QUERY_FORBIDDEN`, `E_QUERY_INVALID` (v0.47, C.5.2), `E_TIMEOUT`, `E_LOCKED`.
...
### C.1 `locate(query: string, k: int = 5, scope: "all"|"branches"|"bananas" = "all", type_filter?: string, include?: [string]) → LocateResult`
...
`all | branches | bananas` is `E_SCHEMA` naming the parameter, the value
...
1. **The convention is declared, never guessed.** `gardener.yaml` MAY
   carry an `aliases:` map of folder name → prefix
   (`back-end: BE`). No map, no aliases: the engine stays forest-agnostic
   (the convention is content vocabulary, and content vocabulary lives in
   the forest's own config — the G.4.5 rule wearing another hat).
2. **The derivation is mechanical.** For a source file whose stem starts
   with digits `N`, sitting in a mapped folder `F` with prefix `P`, the
   draft gains `aliases: ["P-N", "F/N"]` — the conversational form and the
   path form, both of which integrators were observed to try. An unmapped
   folder or an unnumbered file derives nothing.
3. **`sync` keeps them.** A refresh preserves curated frontmatter (G.3),
   aliases included; new files go through the draft build and derive
   theirs. A changed convention is applied by re-adopting or by `graft` —
   never by a refresh silently rewriting names somebody may have edited.
...
- An upload entry MAY carry `source_url` (v0.48): where these bytes came
  ...
  page, a saved image. `http`/`https` only, at most 2048 characters;
...
- **The cap is also said.** When set, the effective value replaces
  ...
  model shapes the reply to fit rather than being truncated by it. On a
  walk it bounds every turn (a tool call is far under the floor) and the
  prompt names it for the final answer.
