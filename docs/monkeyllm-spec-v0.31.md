# MonkeyLLM Technical Specification v0.31 (Phase 0/1/2 + host layer)
...
The three MUST account for the host's whole span, so a caller that
...
`model`, and the three MUST sum to no more than the host's measured span
