# MonkeyLLM Technical Specification v0.50 (Phase 0/1/2 + host layer)
...
- **On `E_LOCKED` the Clipper queues client-side and retries.** The
  ...
  Studio's (J.9.2).
- **Capture offers the visible view and a dragged region.** The region
  picker is an overlay injected on the click the same activeTab
...
- **It injects on user action only.** The extension requests host
  ...
  the screenshot of what was on screen.
