# MonkeyLLM Technical Specification v0.33 (Phase 0/1/2 + host layer)
...
- `model` the provider round trip, present only when one happened
  (J.10.3, J.10.5).
- `cache` the answer store's own work: the lookup, and on a hit the heat
  deposit (J.10.7). Present only when the store was consulted. On a hit
  `model` is absent, because no provider ran a header claiming one did
  would be the forged number this section exists to prevent.
...
### J.10.7 The answer already given (v0.33)
...
**The store fronts `answer` and nothing else.** Primitives are never
...
saving. `curate` is never served from a store, because its purpose is a
fresh reading. Wherever `answer` is served, the store sits in front of it.
Per forest it can be switched off; it is **on by default with a stated
bound**, because the key below makes a stale hit structurally impossible
and the labelling below makes every hit visible.
...
3. `k`, the hops budget when the walk was on (J.10.5), and the
   entry-search mode (K.3) a hybrid entry retrieves differently, so it
   answers differently;
...
**The forest's own clock is the invalidation.** Every change to a forest
is a commit `plant`, `graft`, `tend`, the Gardener's sync, the Ranger's
promote and prune and HEAD is in the key, so every entry made before a
write misses after it. There is no invalidation code to be wrong and no
staleness window to tune. Evaporation (H.1) commits nothing and changes no
answer, and correctly invalidates nothing. A TTL exists as hygiene space,
never correctness: an entry is valid for exactly as long as its key can
still be formed.
...
**A hit is a record served, and it says so.** The stored response returns
as received, with `cached: true` and the time of the original run. Its
trace, usage and cost describe the run that produced it a record, not a
bill, which is J.5.9's distinction one level down. A console MUST label
the answer as served from the store, and the original cost MUST NOT be
counted a second time as spend. The host's log states the hit, with the
entry's digest.

**A hit is still a use of the forest.** An entry keeps the trail of the
run that made it the nodes the sweep was built from, or the nodes the
walk opened. Serving the entry MUST deposit heat on that trail, and MUST
deposit it through the trails store directly, never through a primitive.
This is J.6.1's warming rule in mirror: warming through `locate` would
forge evidence of use, and serving from a store without depositing would
hide it and the nodes that answer a deployment's most-asked questions
are precisely the ones the Ranger must never read as cold (H.2). No tracer
event is recorded, because no primitive ran: pheromone says the knowledge
is in use, which is true; a trace step would say a call ran, which is not.
...
`cache: false` MUST skip the read, run the model, and **replace** the
...
component of the key still matching exactly**: scope, HEAD, binding, `k`,
hops. The question and the terms derived from it are what similarity
...
true`, the original run's answer, a `Server-Timing` with `cache` and no
`model`, an audit row marked as served from the store, and heat on the
entry's trail measurably risen with no tracer event appended. A `plant`
between the two asks makes it two provider calls. A change to any of:
terms, `k`, hops, binding, or scope is a miss. A run with empty evidence,
asked twice, is two provider calls and no entry. `cache: false` answers
fresh and replaces the entry. The store never exceeds its stated bound and
evicts oldest-served-first. An entry stored under one scope is never
served to another. With the near tier off, a rephrased question is a miss;
with it on, a neighbour served names the stored question it answered.
Covered by tests.
